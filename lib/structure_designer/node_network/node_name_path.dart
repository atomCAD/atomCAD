...
/// Placeholder for a node with no `custom_name`. Every node has one in
/// practice — `add_node` mints one and the `.cnnd` loader backfills — so this
/// exists only so a path is never silently truncated.
String _nameOf(NodeView node) {
...
String nodeNamePath(
  ...
  return [...scopeOwnerNames(root, scopeChain), _nameOf(node)].join('/');
}
