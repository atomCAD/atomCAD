...
- **Live drag-sync** — the Default-tool drag handler (`_AtomEditDefaultDelegate.onPrimaryMove`) only calls `renderingNeeded()` (GPU redraw), not a model notify, so the panel wouldn't re-read guideline state until pointer-up. To make the position field / off-line readout track the atom *during* a drag, the `dragging` branch calls `model.notifyGuidelineDragSync()` (a plain `notifyListeners()`, no kernel re-fetch — the card re-reads via FFI) when `getAtomEditGuideline().subMode == move`. Gated to Move sub-mode so normal/Place drags skip the extra rebuild. On the Rust side, `build_api_guideline` reports the atom's **live projection** `t` in Move sub-mode (so a not-snapped drag reflects too; snapped keeps the stored `t` to avoid re-projection float noise).
