...
/// Editor for the `mat3_cols` node. Lays out the stored 3x3 float matrix as
/// three columns; each column is overridden by the corresponding wired input
/// pin (`a` → col 0, `b` → col 1, `c` → col 2).
class Mat3ColsEditor extends StatelessWidget {
  final BigInt nodeId;
  final APIMat3ColsData? data;
  final StructureDesignerModel model;

  const Mat3ColsEditor({
    super.key,
      ...
      child: Column(
        ...
        children: [
          const NodeEditorHeader(
            title: 'Mat3 Cols',
            nodeTypeName: 'mat3_cols',
          ),
          const SizedBox(height: 8),
          const Text(
            'Each column is one column of the output 3x3 float matrix. A '
  ...
  Widget _bodyRow({
    ...
    required List<List<double>> cols,
  ...
  void _updateCell(int colIndex, int rowIndex, double value) {
    final cols = [
      [data!.a.x, data!.a.y, data!.a.z],
      [data!.b.x, data!.b.y, data!.b.z],
      [data!.c.x, data!.c.y, data!.c.z],
    ...
    model.setMat3ColsData(
      nodeId,
      APIMat3ColsData(
        a: APIVec3(x: cols[0][0], y: cols[0][1], z: cols[0][2]),
        b: APIVec3(x: cols[1][0], y: cols[1][1], z: cols[1][2]),
        c: APIVec3(x: cols[2][0], y: cols[2][1], z: cols[2][2]),
      ),
    );
  }
