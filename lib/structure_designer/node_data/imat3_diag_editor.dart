...
/// Editor for the `imat3_diag` node. Renders the stored diagonal vector `v`
/// as three cells; the input pin overrides the whole vector when wired.
class IMat3DiagEditor extends StatelessWidget {
  final BigInt nodeId;
  final APIIMat3DiagData? data;
  final StructureDesignerModel model;

  const IMat3DiagEditor({
    super.key,
      ...
      child: Column(
        ...
        children: [
          const NodeEditorHeader(
            title: 'IMat3 Diagonal',
            nodeTypeName: 'imat3_diag',
          ),
          const SizedBox(height: 8),
          Text(
            connected
                ? 'v pin is connected — the stored diagonal is overridden by '
                    'the wired IVec3 at evaluation.'
  ...
  void _updateCell(int idx, int value) {
    final v = [data!.v.x, data!.v.y, data!.v.z];
    v[idx] = value;
    model.setImat3DiagData(
      nodeId,
      APIIMat3DiagData(v: APIIVec3(x: v[0], y: v[1], z: v[2])),
    );
  }
