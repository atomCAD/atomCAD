...
fn test_sphere_center_is_negative() {
    ...
    assert!(center_value < 0.0, "Center of sphere should be inside (negative SDF)");
    assert!((center_value + 1.0).abs() < EPSILON, "SDF at center should be -radius");
}
...
fn test_sphere_surface_is_zero() {
    ...
    assert!(on_surface_x.abs() < EPSILON, "Point on +X surface should be ≈ 0");
    assert!(on_surface_y.abs() < EPSILON, "Point on +Y surface should be ≈ 0");
    assert!(on_surface_z.abs() < EPSILON, "Point on +Z surface should be ≈ 0");
}
...
fn test_sphere_outside_is_positive() {
    ...
    assert!((outside - 2.0).abs() < EPSILON, "SDF at distance 3 should be 2");
}
...
fn test_sphere_with_offset_center() {
    ...
    assert!((at_origin - 4.0).abs() < EPSILON, "SDF at origin should be 4");
...
fn test_half_space_with_offset() {
    ...
    assert!((at_origin + 5.0).abs() < EPSILON, "SDF at origin should be -5");
}
...
fn test_circle_surface_is_zero() {
    ...
    assert!(on_surface_x.abs() < EPSILON, "Point on +X edge should be ≈ 0");
    assert!(on_surface_y.abs() < EPSILON, "Point on +Y edge should be ≈ 0");
}
...
fn test_circle_outside_is_positive() {
    ...
    assert!((outside - 3.0).abs() < EPSILON, "SDF at distance 4 should be 3");
}
...
fn test_half_plane_basic() {
    ...
    assert!(above > 0.0, "Points above line should be outside (positive SDF)");
    
    // Points below (in -Y) are opposite to normal = negative = inside
    let below = half_plane.implicit_eval_2d(&DVec2::new(0.5, -1.0));
    assert!(below < 0.0, "Points below line should be inside (negative SDF)");
...
fn test_sphere_gradient() {
    ...
    assert!((value - 1.0).abs() < EPSILON, "Value should be distance - radius = 1.0");
}
...
fn test_union_3d_empty() {
    ...
    assert!(value > 1e10, "Empty union should return very large positive value");
}
...
fn test_intersection_3d_empty() {
    ...
    assert!(value < -1e10, "Empty intersection should return very large negative value");
}
