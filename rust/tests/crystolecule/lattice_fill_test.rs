use rust_lib_flutter_cad::crystolecule::lattice_fill::{
    PlacedAtomTracker, CrystallographicAddress, 
    LatticeFillConfig, LatticeFillOptions, LatticeFillStatistics,
    fill_lattice
};
...
";
    ...
    assert_eq!(result.atomic_structure.get_num_of_atoms(), 0, "Should have no atoms when geometry doesn't overlap");
}
...
";
    ...
    assert!(result.atomic_structure.get_num_of_atoms() > 0, "Should have placed atoms");
