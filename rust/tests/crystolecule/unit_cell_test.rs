...
mod unit_cell_tests {
    ...
    fn test_zero_vector_conversion() {
        ...
        assert!(
            zero_real.length() < 1e-10,
            "Zero lattice should map to zero real"
        );
        assert!(
            recovered_lattice.length() < 1e-10,
            "Zero real should map to zero lattice"
        );
    }
    ...
    fn test_miller_index_plane_properties() {
        ...
        for miller_index in test_indices {
            ...
            assert!(
                plane_props.d_spacing > 0.0,
                "d-spacing should be positive for {:?}, got {}",
                miller_index,
                plane_props.d_spacing
            );
            ...
            if miller_index == IVec3::new(1, 0, 0) {
                ...
                assert!(
                    diff < 1e-10,
                    "(100) normal should be (1,0,0), got {:?}",
                    plane_props.normal
                );
            }
        }
    ...
    fn test_numerical_precision() {
        ...
        assert!(
            small_diff < 1e-15,
            "Small value precision test failed: diff = {}",
            small_diff
        );
        ...
        assert!(
            large_diff < 1e-6,
            "Large value precision test failed: diff = {}",
            large_diff
        );
    }
