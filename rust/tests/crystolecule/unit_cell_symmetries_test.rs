...
use rust_lib_flutter_cad::crystolecule::unit_cell_symmetries::{
    CrystalSystem, RotationalSymmetry, analyze_unit_cell_complete, analyze_unit_cell_symmetries,
    classify_crystal_system,
};

/// Helper function to create a test unit cell with given parameters
fn create_test_unit_cell(
    a: f64,
    b: f64,
    c: f64,
    alpha: f64,
...
fn validate_symmetries_preserve_lattice(unit_cell: &UnitCellStruct, test_name: &str) {
    ...
    println!(
        "Testing {} with {} symmetries and {} lattice points",
        test_name,
        symmetries.len(),
        test_points.len()
    );
            ...
            for (point_idx, &lattice_point) in test_points.iter().enumerate() {
                ...
                if !is_close_to_integer_coordinates(rotated_lattice_point, tolerance) {
                    panic!(
                        ...
                        real_point.y,
                        real_point.z,
                        rotated_real_point.x,
                        rotated_real_point.y,
                        rotated_real_point.z,
                        rotated_lattice_point.x,
                        rotated_lattice_point.y,
                        rotated_lattice_point.z,
