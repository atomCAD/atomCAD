...
fn test_auto_connect_output_to_compatible_input() {
    ...
    assert!(
        result,
        "Should successfully auto-connect Float output to Vec3 input"
    );
    ...
    assert!(
        !vec3_node.arguments[0].is_empty(),
        "Vec3's first argument (x) should be connected"
    );
...
fn test_auto_connect_input_to_compatible_output() {
    ...
    assert!(
        result,
        "Should successfully auto-connect from Vec3 input to Float node's output"
    );
    ...
    assert!(
        !vec3_node.arguments[0].is_empty(),
        "Vec3's first argument should be connected"
    );
    assert_eq!(vec3_node.arguments[0].get_node_id(), Some(float_id));
}
...
fn test_auto_connect_int_to_int_input() {
    ...
    assert!(
        !sphere_node.arguments[1].is_empty(),
        "Sphere's radius argument should be connected"
    );
...
fn test_auto_connect_int_promotable_to_float() {
    ...
    assert!(
        result,
        "Int should be promotable to Float for Vec3's inputs"
    );
...
fn test_auto_connect_incompatible_types_fails() {
    ...
    assert!(
        !result,
        "String should not be connectable to Sphere's Float inputs"
    );
...
fn test_auto_connect_finds_first_compatible_input() {
    ...
    let connected_param = geo_trans_node
        .arguments
        .iter()
...
fn test_auto_connect_from_input_to_compatible_output() {
    ...
    assert!(
        result,
        "Float output should connect to Float input when dragging from input"
    );
...
fn test_get_compatible_pins_geometry_to_geometry_input() {
    ...
    assert_eq!(
        pins.len(),
        1,
        "Expected 1 compatible pin (shapes), got {}",
        pins.len()
    );
...
fn test_get_compatible_pins_returns_all_compatible_not_just_first() {
    ...
    assert_eq!(
        pins.len(),
        3,
        "Expected 3 compatible pins (x, y, z), got {}",
        pins.len()
    );
...
fn test_rename_node_network_prevents_builtin_name_conflict() {
    ...
    assert_eq!(
        result, false,
        "Should not allow renaming to built-in node type 'parameter'"
    );
    ...
    assert_eq!(
        result, false,
        "Should not allow renaming to built-in node type 'string'"
    );
    ...
    assert_eq!(
        result, false,
        "Should not allow renaming to built-in node type 'int'"
    );
    ...
    assert!(
        designer
            .node_type_registry
            .node_networks
            .contains_key("my_custom_network")
    );
    assert!(
        !designer
            .node_type_registry
...
fn test_rename_node_network_existing_validations_still_work() {
    ...
    assert_eq!(
        result, false,
        "Should not allow renaming to existing network name"
    );
    ...
    assert_eq!(
        result, false,
        "Should not allow renaming non-existent network"
    );
...
fn test_rename_node_network_updates_backtick_references_in_comments() {
    ...
    // Set comment data with backtick references to MyModule using text properties
    {
        ...
        props.insert(
            "label".to_string(),
            TextValue::String("About `MyModule`".to_string()),
        );
    ...
    assert_eq!(
        label,
        Some(&TextValue::String("About `CoreModule`".to_string())),
        "Label backtick reference should be updated"
    );
...
fn test_rename_node_network_preserves_non_matching_backticks() {
    ...
    // Set comment text using text properties
    {
        ...
        props.insert(
            "text".to_string(),
            TextValue::String(
                "Use `MyNet` here. See also `MyNetwork` and `code` examples.".to_string(),
            ),
        );
        node.data.set_text_properties(&props).unwrap();
    }
    ...
    assert_eq!(
        text,
        Some(&TextValue::String(
            "Use `RenamedNet` here. See also `MyNetwork` and `code` examples.".to_string()
        )),
...
fn test_new_project_clears_networks() {
    ...
    assert!(
        !designer
            .node_type_registry
            .node_networks
            .contains_key("network1")
    );
    assert!(
        !designer
            .node_type_registry
            .node_networks
            .contains_key("network2")
    );
    assert!(
        !designer
            .node_type_registry
            .node_networks
            .contains_key("network3")
    );
}
