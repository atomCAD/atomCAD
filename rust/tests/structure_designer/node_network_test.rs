...
fn test_get_connected_node_ids() {
    ...
    assert!(
        connected.contains(&float_id),
        "Sphere should be connected to float"
    );
    assert!(
        connected.contains(&union_id),
        "Sphere should be connected to union"
    );
...
fn test_move_selected_nodes() {
    ...
    assert_eq!(
        network.nodes.get(&node1).unwrap().position,
        DVec2::new(50.0, 25.0)
    );
    assert_eq!(
        network.nodes.get(&node2).unwrap().position,
        DVec2::new(150.0, 125.0)
    );
    // node3 should NOT be moved (not selected)
    assert_eq!(
        network.nodes.get(&node3).unwrap().position,
...
fn test_add_node_assigns_unique_names_per_type() {
    ...
    let network = designer
        ...
        .unwrap();
    assert_eq!(
        network.nodes.get(&sphere_id).unwrap().custom_name,
        Some("sphere1".to_string())
    );
    assert_eq!(
        network.nodes.get(&cuboid_id).unwrap().custom_name,
        Some("cuboid1".to_string())
    );
    assert_eq!(
        network.nodes.get(&sphere2_id).unwrap().custom_name,
        Some("sphere2".to_string())
    );
}
