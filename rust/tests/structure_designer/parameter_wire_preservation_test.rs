...
fn set_parameter_name(designer: &mut StructureDesigner, network_name: &str, node_id: u64, new_name: &str) {
    designer.set_active_node_network_name(Some(network_name.to_string()));
    let network = designer.node_type_registry.node_networks.get_mut(network_name).unwrap();
    let node = network.nodes.get_mut(&node_id).unwrap();
    if let Some(param_data) = node.data.as_any_mut().downcast_mut::<ParameterData>() {
        let mut props = HashMap::new();
        props.insert("param_name".to_string(), TextValue::String(new_name.to_string()));
        props.insert("data_type".to_string(), TextValue::DataType(param_data.data_type.clone()));
        props.insert("sort_order".to_string(), TextValue::Int(param_data.sort_order));
        props.insert("param_index".to_string(), TextValue::Int(param_data.param_index as i32));
...
fn set_parameter_sort_order(designer: &mut StructureDesigner, network_name: &str, node_id: u64, sort_order: i32) {
    designer.set_active_node_network_name(Some(network_name.to_string()));
    let network = designer.node_type_registry.node_networks.get_mut(network_name).unwrap();
    let node = network.nodes.get_mut(&node_id).unwrap();
    if let Some(param_data) = node.data.as_any_mut().downcast_mut::<ParameterData>() {
        let mut props = HashMap::new();
        props.insert("param_name".to_string(), TextValue::String(param_data.param_name.clone()));
        props.insert("data_type".to_string(), TextValue::DataType(param_data.data_type.clone()));
...
fn set_parameter_type(designer: &mut StructureDesigner, network_name: &str, node_id: u64, data_type: DataType) {
    designer.set_active_node_network_name(Some(network_name.to_string()));
    let network = designer.node_type_registry.node_networks.get_mut(network_name).unwrap();
    let node = network.nodes.get_mut(&node_id).unwrap();
    if let Some(param_data) = node.data.as_any_mut().downcast_mut::<ParameterData>() {
...
fn get_parameter_names(designer: &StructureDesigner, network_name: &str) -> Vec<String> {
    let network = designer.node_type_registry.node_networks.get(network_name).unwrap();
    network.node_type.parameters.iter().map(|p| p.name.clone()).collect()
}
...
fn test_subnetwork_add_parameter_preserves_existing_wires() {
    ...
    assert_eq!(get_wire_count(&designer, "main", filter_id, 1), 0,
...
fn test_subnetwork_add_parameter_in_middle_repositions_wires() {
    ...
    assert_eq!(get_parameter_names(&designer, "MyFilter"), vec!["first", "last"]);
    ...
    assert!(has_wire_from(&designer, "main", filter_id, 0, int1_id), "first wire should exist");
    assert!(has_wire_from(&designer, "main", filter_id, 1, int2_id), "last wire should exist");
    ...
    assert_eq!(get_parameter_names(&designer, "MyFilter"), vec!["first", "middle", "last"]);
    ...
    assert!(has_wire_from(&designer, "main", filter_id, 2, int2_id),
...
fn test_subnetwork_reorder_parameters_wires_follow_names() {
    ...
    assert_eq!(get_parameter_names(&designer, "MyFilter"), vec!["size", "radius"]);
    ...
    assert!(has_wire_from(&designer, "main", filter_id, 0, int_size), "size wire at index 0");
    assert!(has_wire_from(&designer, "main", filter_id, 1, int_radius), "radius wire at index 1");
    ...
    assert_eq!(get_parameter_names(&designer, "MyFilter"), vec!["radius", "size"]);

    // After reorder: radius at index 0, size at index 1
    // Wires should follow their parameter names to new positions
    assert!(has_wire_from(&designer, "main", filter_id, 0, int_radius),
        "radius wire should be at index 0 after reorder");
    assert!(has_wire_from(&designer, "main", filter_id, 1, int_size),
...
fn test_subnetwork_change_parameter_type_preserves_wire() {
    ...
    assert!(has_wire_from(&designer, "main", filter_id, 0, float_id), "Float wire should exist");
    ...
    assert!(has_wire_from(&designer, "main", filter_id, 0, float_id),
...
fn test_subnetwork_multiple_parents_all_repaired() {
    ...
    assert_eq!(get_wire_count(&designer, "parentA", filter_a, 1), 0,
        "parentA new param should have no wire");

    assert!(has_wire_from(&designer, "parentB", filter_b, 0, int_b),
        "parentB wire should be preserved");
    assert_eq!(get_wire_count(&designer, "parentB", filter_b, 1), 0,
...
fn update_expr_parameters(designer: &mut StructureDesigner, network_name: &str, node_id: u64,
                         params: Vec<(&str, DataType)>, expression: &str) {
    designer.set_active_node_network_name(Some(network_name.to_string()));
    let network = designer.node_type_registry.node_networks.get_mut(network_name).unwrap();
    let node = network.nodes.get_mut(&node_id).unwrap();
    ...
    if let Some(expr_data) = node.data.as_any_mut().downcast_mut::<ExprData>() {
        ...
        let params_arr: Vec<TextValue> = params.iter().map(|(name, dt)| {
        ...
        let mut props = HashMap::new();
        props.insert("expression".to_string(), TextValue::String(expression.to_string()));
        ...
        let base_node_type = designer.node_type_registry.built_in_node_types.get("expr").unwrap().clone();
...
fn test_expr_add_parameter_preserves_existing_wires() {
    ...
    assert!(has_wire_from(&designer, "main", expr_id, 0, int_id), "Initial wire should exist");

    // Add a second parameter "y"
    update_expr_parameters(&mut designer, "main", expr_id,
    ...
    assert_eq!(get_wire_count(&designer, "main", expr_id, 1), 0,
...
fn test_expr_reorder_parameters_wires_follow_names() {
    ...
    assert!(has_wire_from(&designer, "main", expr_id, 0, int_y),
        "Wire to y should now be at index 0");
    assert!(has_wire_from(&designer, "main", expr_id, 1, int_x),
...
fn test_subnetwork_rename_parameter_preserves_wire() {
    ...
    assert_eq!(get_parameter_names(&designer, "MyFilter"), vec!["length"],
