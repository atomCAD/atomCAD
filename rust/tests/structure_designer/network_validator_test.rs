...
fn test_validate_empty_network() {
    ...
    assert!(network.validation_errors.is_empty(), "Empty network should have no validation errors");
}
...
fn test_validate_connected_nodes() {
    ...
    let network = designer.node_type_registry.node_networks.get("test_network").unwrap();
    assert!(network.valid, "Network with valid connections should be valid");
}
...
fn test_validate_network_with_return_node() {
    ...
    let network = designer.node_type_registry.node_networks.get("test_network").unwrap();
    assert!(network.valid, "Network with valid return node should be valid");
...
fn test_add_and_validate_parameter_node() {
    ...
    assert_eq!(network.node_type.parameters.len(), 1, "Should have one parameter");
}
...
fn test_validate_with_multiple_parameters() {
    ...
    assert!(network.valid, "Network with multiple unique parameters should be valid");
    assert_eq!(network.node_type.parameters.len(), 3, "Should have three parameters");
}
