...
impl std::ops::Mul<IVec3> for IMat3 {
    type Output = IVec3;
...
impl std::ops::Mul<IMat3> for IMat3 {
    type Output = IMat3;

    fn mul(self, rhs: IMat3) -> Self::Output {
        let x_axis = self.mul(&rhs.cols[0]);
        let y_axis = self.mul(&rhs.cols[1]);
        let z_axis = self.mul(&rhs.cols[2]);
        
        IMat3::new(&x_axis, &y_axis, &z_axis)
    }
}
