...
pub fn convert_csg_sketch_to_poly_mesh(
    csg_sketch: CSGSketch,
    triangulate_2d: bool,
    ...
    let u_real = drawing_plane
        .unit_cell
        .ivec3_lattice_to_real(&drawing_plane.u_axis);
    let v_real = drawing_plane
        .unit_cell
        .ivec3_lattice_to_real(&drawing_plane.v_axis);
    let plane_normal = u_real.cross(v_real).normalize();
    let plane_origin = drawing_plane
        .unit_cell
        .ivec3_lattice_to_real(&drawing_plane.center);
...
fn convert_polygons_to_poly_mesh(
    ...
    for polygon in polygons {
        ...
        for vertex in &polygon.vertices {
            ...
            let vertex_idx =
                unique_vertices.get_or_insert(position, poly_mesh.add_vertex(position));
        ...
        let vertices = [
            Vertex::new(
                ...
                nalgebra::Vector3::new(
                    plane_normal.x as Real,
                    plane_normal.y as Real,
                    plane_normal.z as Real,
                ),
            ),
            Vertex::new(
