...
fn add_hatched_quad(
    ...
    for u in 0..grid_count {
        for v in 0..grid_count {
            ...
            let vertex_indices: Vec<u32> = corners
                .iter()
                .map(|pos| mesh.add_vertex(Vertex::new(pos, &normal, material)))
                .collect();
...
fn tessellate_poly_mesh_smooth(
    poly_mesh: &PolyMesh,
    mesh: &mut Mesh,
    material: &Material,
    tessellate_outside: bool,
    ...
    let vertex_indices: Vec<u32> = poly_mesh
        .vertices
        .iter()
        .enumerate()
        .map(|(idx, vertex)| {
...
fn tessellate_poly_mesh_sharp(
    poly_mesh: &PolyMesh,
    mesh: &mut Mesh,
    material: &Material,
    highlighted_material: Option<&Material>,
    tessellate_outside: bool,
) {
    // Process each face
    ...
    for face in &poly_mesh.faces {
        ...
        for &vertex_idx in &face.vertices {
            ...
            let mesh_vertex_idx =
                mesh.add_vertex(Vertex::new(&vertex_position, &normal, face_material));
...
/// * `mesh` - The target mesh to add vertices and faces to
/// * `material` - The material to apply to the mesh vertices
fn tessellate_poly_mesh_smoothing_group_based(
    poly_mesh: &PolyMesh,
    mesh: &mut Mesh,
    material: &Material,
    tessellate_outside: bool,
...
pub fn tessellate_poly_mesh(
    poly_mesh: &PolyMesh,
    mesh: &mut Mesh,
    smoothing: MeshSmoothing,
    outside_material: &Material,
    inside_material: Option<&Material>,
    highlighted_material: Option<&Material>,
) {
...
fn tessellate_poly_mesh_one_sided(
    ...
    if poly_mesh.hatched && poly_mesh.faces.len() == 1 && poly_mesh.faces[0].vertices.len() == 4 {
        ...
        add_hatched_quad(
            ...
            normals,
            material,
            crystolecule_constants::DIAMOND_UNIT_CELL_SIZE_ANGSTROM as f32 * 0.5,
        );
        return;
    }
    ...
    let effective_smoothing = if has_highlighted_faces {
        MeshSmoothing::Sharp
    } else {
        smoothing
    };
    ...
    match effective_smoothing {
        MeshSmoothing::Smooth => {
            tessellate_poly_mesh_smooth(poly_mesh, mesh, material, tessellate_outside)
        }
        MeshSmoothing::Sharp => tessellate_poly_mesh_sharp(
            poly_mesh,
            mesh,
            material,
            highlighted_material,
        ...
        MeshSmoothing::SmoothingGroupBased => tessellate_poly_mesh_smoothing_group_based(
            poly_mesh,
            mesh,
            material,
            tessellate_outside,
...
pub fn tessellate_poly_mesh_to_line_mesh(
    ...
    for ((v1_idx, v2_idx), edge) in &poly_mesh.edges {
        ...
        if processed_edges.contains(&(*v1_idx, *v2_idx))
            || processed_edges.contains(&(*v2_idx, *v1_idx))
        {
            continue;
        }
