...
- **No →** keep it `ValidationError::new` (blocking). The validation pass is the *only* thing standing between the user and a panic/hang **or a silently-wrong result**. Worked examples that stay blocking: a **type mismatch** (`convert_to` passes the wrong-typed value through *unchanged* — no `Error` — so downstream `extract_*().unwrap()` can panic / emit garbage); **`apply` non-contiguous arg pins** (eval silently drops wires past the gap → wrong value, not an `Error`); **parameter rules** (they corrupt the network's *interface*, so the blast radius is call-sites in other networks, not local). Note: all of `validate_wires` / `validate_parameters`' current rules are blocking, but since error-management Phase 2 (`doc/design_error_management.md` D4) both passes **accumulate** — `validate_wires` records each node's first error (sorted-node-id order, exact-duplicate rows deduped) and moves on to the next node; `validate_parameters` records every cast/duplicate-name/abstract-type violation but still skips the interface rebuild when any exists. A new non-blocking rule is therefore no longer structurally confined to `validate_zones_recursive`.
