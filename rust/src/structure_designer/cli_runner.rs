...
pub fn run_cli_single_mode(
    ...
    designer
        .load_node_networks(&config.cnnd_file)
...
fn validate_network_exists(designer: &StructureDesigner, network_name: &str) -> Result<(), String> {
    io::stdout().flush().ok();
    ...
    if !available_networks.contains(&network_name.to_string()) {
        eprintln!(
            "ERROR: Network '{}' not found in loaded file.",
            network_name
        );
...
fn parse_cli_parameters(
    ...
    for (param_name, value_str) in parameters {
        ...
        let param_def = node_type
            .parameters
            .iter()
            .find(|p| &p.name == param_name)
            .ok_or_else(|| {
                format!(
                    "Parameter '{}' not found in network '{}'",
