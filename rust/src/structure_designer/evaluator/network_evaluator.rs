...
impl NetworkEvaluator {
  ...
  pub fn generate_scene(&self, network_name: &str, node_id: u64, registry: &NodeTypeRegistry) -> StructureDesignerScene {
    ...
    if node_type.output_type == DataType::Geometry2D {
      return self.generate_2d_point_cloud_scene(network, node_id, registry, &mut context);
    }
    if node_type.output_type == DataType::Geometry {
  ...
  pub fn generate_2d_point_cloud_scene(&self, network: &NodeNetwork, node_id: u64, registry: &NodeTypeRegistry, context: &mut NetworkEvaluationContext) -> StructureDesignerScene {
    ...
    self.process_rect_for_point_cloud(
        network,
        node_id,
        registry,
        &(common_constants::IMPLICIT_VOLUME_MIN.xz() * SAMPLES_PER_UNIT),
        &((common_constants::IMPLICIT_VOLUME_MAX.xz() - common_constants::IMPLICIT_VOLUME_MIN.xz()) * SAMPLES_PER_UNIT),
        &mut eval_cache,
        &mut point_cloud);
  ...
  // Evaluate SDF at the center point
  let sdf_value = self.implicit_evaluator.eval_2d(network, node_id, &center_point, registry)[0];
  ...
  if !should_subdivide_x && !should_subdivide_y {
      ...
      for x in 0..size.x {
          for y in 0..size.y {
                  ...
                  self.process_2d_cell_for_point_cloud(
                      network,
                      node_id,
                      registry,
                      &cell_pos,
  ...
  point_cloud: &mut SurfacePointCloud2D) {
    ...
    let values: Vec<f64> = corner_points.iter().map(|ip| {
      if let Some(&cached_value) = eval_cache.get(ip) {
        ...
        let value = self.implicit_evaluator.eval_2d(network, node_id, &p, registry)[0];
        //println!("Evaluating point: {:?}, value: {}", ip, value);
        eval_cache.put(*ip, value);
        value
      }
    ...
    if values.iter().any(|&v| v >= 0.0) && values.iter().any(|&v| v < 0.0) {
        ...
        let gradient_val = self.implicit_evaluator.get_gradient_2d(network, node_id, &center_point, registry);
        let gradient = gradient_val.0;
        let value = gradient_val.1;
        let gradient_magnitude_sq = gradient.length_squared();
        // Avoid division by very small numbers
