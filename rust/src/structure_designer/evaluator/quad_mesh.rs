...
impl QuadMesh {
    ...
    pub fn add_quad(&mut self, v0: usize, v1: usize, v2: usize, v3: usize) -> usize {
        ...
        for &vertex_index in &[v0, v1, v2, v3] {
            ...
            if vertex_index < self.vertices.len() {
                self.vertices[vertex_index].quad_indices.push(quad_index);
            }
        }
    ...
    pub fn compute_quad_normals(&mut self) {
        for quad in &mut self.quads {
            ...
            let v0 = self.vertices[quad.vertices[0]].position;
            let v1 = self.vertices[quad.vertices[1]].position;
            let v2 = self.vertices[quad.vertices[2]].position;
    ...
    pub fn get_vertex_position(&self, index: usize) -> Option<&DVec3> {
        self.vertices.get(index).map(|v| &v.position)
    }
    
    /// Sets the position of a vertex
    pub fn set_vertex_position(&mut self, index: usize, position: DVec3) {
    ...
    pub fn get_vertex_quad_indices(&self, vertex_index: usize) -> &[usize] {
        if vertex_index < self.vertices.len() {
            &self.vertices[vertex_index].quad_indices
        } else {
            &[]
        }
    }
