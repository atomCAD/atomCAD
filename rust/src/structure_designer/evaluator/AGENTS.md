...
- `Function(ZoneClosure)` is a first-class function value — the same detached zone-body bundle (`evaluator/zone_closure.rs`: `{ body, captures, zone_output_wires, owner_node_id, param_types, return_type }`) that an inline HOF body carries, handed around as a value. `infer_data_type` derives its `DataType::Function` from the carried `param_types`/`return_type` (`ZoneClosure::function_type`). As of Phase 2 no node yet *produces* this variant — it is valid-but-unconstructed until the `closure` node lands in Phase 3. See `doc/design_closures.md`.
