...
- **Variants** (all in `iterator_walker.rs`): `FromArray { items: Arc<Vec<NetworkResult>>, idx }`, `Range { start, step, count, emitted }`, `Map { source, fe }`, `Filter { source, fe }`, `Product { axes, field_names, current, primed, done }`, `MapZone { source, body, zone_output_wires, ... }`, `FilterZone { source, body, zone_output_wires, ... }`. The `Zone` variants are the inline-body forms of `map` / `filter`: they carry the HOF's owned body `Arc<NodeNetwork>` plus a snapshot of the body-return wires (`zone_output_wires`) and push a scope frame on each `next` to evaluate the body per element. See "Zones" in `../AGENTS.md` for the design.
...
- **Construction-time errors** (closure FE can't be built — source network missing, source node missing) must be detected by `map.eval()` / `filter.eval()` and returned as `EvalOutput::single(NetworkResult::Error(_))` — do **not** construct a degenerate walker, or errors multiply per element. The Zone variants have the analogous rule: when the body itself is malformed (no zone-output wire, missing inner source node, …) construct nothing and return an `Error` from `eval`.

### `evaluate_zone_output`

`network_evaluator::evaluate_zone_output(network_stack, zone_output_wires, registry, context)` is the shared helper for getting one per-iteration result out of a body. The eager HOFs (`fold`, `foreach`) call it inside the drain loop after pushing a body-scope frame with the current `(acc, element)` synthesised onto the HOF's `ZoneInput` pins (see `fresh_inner_for_eager_body` in the same file); `Walker::MapZone`/`FilterZone::next` calls it from the lazy path the same way. The helper resolves the one or more wires terminating at the body's zone-output pin(s) by walking up the scope-stack `ancestors` chain for cross-frame sources (captures, zone-input references) and returning the body's per-iteration value(s).
