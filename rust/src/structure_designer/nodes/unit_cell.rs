...
impl NodeData for UnitCellData {
    ...
    fn get_text_properties(&self) -> Vec<(String, TextValue)> {
        vec![
            (
                "cell_length_a".to_string(),
                TextValue::Float(self.cell_length_a),
            ),
            (
                "cell_length_b".to_string(),
                TextValue::Float(self.cell_length_b),
            ),
            (
                "cell_length_c".to_string(),
                TextValue::Float(self.cell_length_c),
            ),
            (
                "cell_angle_alpha".to_string(),
                TextValue::Float(self.cell_angle_alpha),
            ),
            (
                "cell_angle_beta".to_string(),
                TextValue::Float(self.cell_angle_beta),
            ),
            (
                "cell_angle_gamma".to_string(),
                TextValue::Float(self.cell_angle_gamma),
            ),
        ]
    }
    ...
    fn set_text_properties(&mut self, props: &HashMap<String, TextValue>) -> Result<(), String> {
        if let Some(v) = props.get("cell_length_a") {
            self.cell_length_a = v
                .as_float()
                .ok_or_else(|| "cell_length_a must be a float".to_string())?;
        }
        if let Some(v) = props.get("cell_length_b") {
            self.cell_length_b = v
                .as_float()
                .ok_or_else(|| "cell_length_b must be a float".to_string())?;
        }
        if let Some(v) = props.get("cell_length_c") {
            self.cell_length_c = v
                .as_float()
                .ok_or_else(|| "cell_length_c must be a float".to_string())?;
        }
        if let Some(v) = props.get("cell_angle_alpha") {
            self.cell_angle_alpha = v
                .as_float()
                .ok_or_else(|| "cell_angle_alpha must be a float".to_string())?;
        }
        if let Some(v) = props.get("cell_angle_beta") {
            self.cell_angle_beta = v
                .as_float()
                .ok_or_else(|| "cell_angle_beta must be a float".to_string())?;
        }
        if let Some(v) = props.get("cell_angle_gamma") {
            self.cell_angle_gamma = v
                .as_float()
                .ok_or_else(|| "cell_angle_gamma must be a float".to_string())?;
        }
        Ok(())
    }
