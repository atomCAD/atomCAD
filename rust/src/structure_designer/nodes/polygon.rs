...
impl NodeData for PolygonData {
    fn provide_gadget(
        ...
        Some(Box::new(PolygonGadget::new(
            &self.vertices,
            &polygon_cache.drawing_plane,
        )))
    }
    ...
    fn get_text_properties(&self) -> Vec<(String, TextValue)> {
        vec![(
            "vertices".to_string(),
            TextValue::Array(self.vertices.iter().map(|v| TextValue::IVec2(*v)).collect()),
        )]
    }
...
impl Tessellatable for PolygonGadget {
    fn tessellate(&self, output: &mut TessellationOutput) {
        ...
        let real_3d_vertices: Vec<DVec3> = self
            .vertices
            .iter()
        ...
        let selected_handle_material = Material::new(
    ...
    fn hit_test(&self, ray_origin: DVec3, ray_direction: DVec3) -> Option<i32> {
        ...
        for (i, &p1_3d) in real_3d_vertices.iter().enumerate() {
            ...
            if cylinder_hit_test(
                &p2_3d,
                &p1_3d,
                common_constants::LINE_RADIUS * common_constants::LINE_RADIUS_HIT_TEST_FACTOR,
                &ray_origin,
                &ray_direction,
            )
            .is_some()
