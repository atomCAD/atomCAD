...
impl NodeData for ParameterData {
    ...
    fn eval<'a>(
        ...
        let parent_node = parent_network_stack
            .last()
            .unwrap()
            .node_network
            .nodes
        ...
        if let Some(custom_data) = parent_node
            .data
            .as_any_ref()
    ...
    fn get_text_properties(&self) -> Vec<(String, TextValue)> {
        let mut props = vec![
            (
                ...
                TextValue::Int(self.param_index as i32),
            ),
            (
                "param_name".to_string(),
                TextValue::String(self.param_name.clone()),
            ),
            (
                "data_type".to_string(),
                TextValue::DataType(self.data_type.clone()),
            ),
    ...
    fn set_text_properties(&mut self, props: &HashMap<String, TextValue>) -> Result<(), String> {
        if let Some(v) = props.get("param_index") {
            self.param_index = v
                .as_int()
                .ok_or_else(|| "param_index must be an integer".to_string())?
                as usize;
        }
        if let Some(v) = props.get("param_name") {
            self.param_name = v
                .as_string()
                .ok_or_else(|| "param_name must be a string".to_string())?
                .to_string();
        }
        ...
        if let Some(v) = props.get("sort_order") {
            self.sort_order = v
                .as_int()
                .ok_or_else(|| "sort_order must be an integer".to_string())?;
        }
        if let Some(v) = props.get("data_type_str") {
            self.data_type_str = Some(
                v.as_string()
                    .ok_or_else(|| "data_type_str must be a string".to_string())?
                    .to_string(),
            );
        }
        Ok(())
