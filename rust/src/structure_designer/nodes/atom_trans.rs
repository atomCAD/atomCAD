...
impl NodeData for AtomTransData {
    ...
    fn set_text_properties(&mut self, props: &HashMap<String, TextValue>) -> Result<(), String> {
        ...
        if let Some(v) = props.get("rotation") {
            self.rotation = v
                .as_vec3()
                .ok_or_else(|| "rotation must be a Vec3".to_string())?;
        }
        Ok(())
    }
...
impl NodeNetworkGadget for AtomTransGadget {
    fn sync_data(&self, data: &mut dyn NodeData) {
        if let Some(atom_trans_data) = data.as_any_mut().downcast_mut::<AtomTransData>() {
            ...
            let relative_rotation_quat =
                self.frame_transform.rotation * self.input_frame_transform.rotation.inverse();
...
impl AtomTransGadget {
    ...
    fn refresh_frame_transform(&mut self) {
        ...
        self.frame_transform = self
            .input_frame_transform
            .apply_lrot_gtrans_new(&Transform::new(self.translation, rotation_quat));
    }
}
