...
pub fn get_active_facet_shell_data(
    structure_designer: &StructureDesigner,
) -> Option<&FacetShellData> {
...
/// - The selected node is not a facet_shell node
/// - The FacetShellData cannot be retrieved or cast
pub fn get_active_facet_shell_data_mut(
    structure_designer: &mut StructureDesigner,
) -> Option<&mut FacetShellData> {
    let selected_node_id = structure_designer.get_selected_node_id_with_type("facet_shell")?;
    ...
    pub fn highlight_selected_facets(&self, poly_mesh: &mut PolyMesh) {
        ...
        for facet in facet_variants {
            ...
            for face in &mut poly_mesh.faces {
                ...
                if face.normal.dot(normal) > 0.998 {
                    // cos(0.05) ≈ 0.998
    ...
    pub fn hit_facet_by_ray(
        ...
        for (cached_index, facet) in self.cached_facets.iter().enumerate() {
            ...
            let plane_props = unit_cell
                .ivec3_miller_index_to_plane_props(&facet.miller_index)
...
impl NodeData for FacetShellData {
    ...
    fn get_text_properties(&self) -> Vec<(String, TextValue)> {
        ...
        let facets_arr: Vec<TextValue> = self
            .facets
            .iter()
            .map(|f| {
        ...
        if let Some(TextValue::Array(facets_arr)) = props.get("facets") {
            ...
            for facet_val in facets_arr {
                if let TextValue::Object(obj) = facet_val {
                    let miller_index = obj
                        .iter()
                        .find(|(k, _)| k == "miller_index")
...
impl Tessellatable for FacetShellGadget {
    fn tessellate(&self, output: &mut TessellationOutput) {
        ...
        if self.miller_index_variants.len() <= 1000 {
            ...
            for miller_index in &self.miller_index_variants {
                half_space_utils::tessellate_shift_drag_handle(
                    ...
                    &self.unit_cell,
                    1,
                ); // subdivision=1 for facet_shell (no subdivision support)
            }
        } else {
            // Log error and skip tessellation if Vec appears corrupted
            eprintln!(
                "Warning: FacetShellGadget miller_index_variants appears corrupted (len={}), skipping tessellation",
                self.miller_index_variants.len()
            );
        }
...
impl Gadget for FacetShellGadget {
    ...
    fn hit_test(&self, ray_origin: DVec3, ray_direction: DVec3) -> Option<i32> {
        ...
        if self.miller_index_variants.len() <= 1000 {
            // Reasonable upper bound
            for (variant_index, miller_index_variant) in
                self.miller_index_variants.iter().enumerate()
            {
            ...
            eprintln!(
                "Warning: FacetShellGadget miller_index_variants appears corrupted in hit test (len={}), skipping hit testing",
                self.miller_index_variants.len()
            );
        }
...
impl NodeNetworkGadget for FacetShellGadget {
    ...
    fn sync_data(&self, data: &mut dyn NodeData) {
        if let Some(facet_shell_data) = data.as_any_mut().downcast_mut::<FacetShellData>() {
            facet_shell_data.facets[facet_shell_data.selected_facet_index.unwrap()].miller_index =
                self.miller_index;
            facet_shell_data.center = self.center;
            facet_shell_data.facets[facet_shell_data.selected_facet_index.unwrap()].shift =
                self.shift;
...
impl FacetShellGadget {
    pub fn get_dragged_miller_index(&self) -> IVec3 {
        if self.dragged_handle_index.is_some()
            && self.dragged_handle_index.unwrap() > 0
            && !self.miller_index_variants.is_empty()
        {
            ...
            if dragged_variant_index < self.miller_index_variants.len()
                && self.miller_index_variants.len() <= 1000
            {
                self.miller_index_variants[dragged_variant_index]
            } else {
                eprintln!(
                    "Warning: FacetShellGadget miller_index_variants bounds check failed (index={}, len={})",
                    dragged_variant_index,
                    self.miller_index_variants.len()
                );
                self.miller_index
            }
