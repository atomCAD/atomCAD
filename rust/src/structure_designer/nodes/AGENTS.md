...
- **Diff output pins (issue #295, `doc/design_diff_outputs_for_atom_ops.md`):** `relax`, the four movement nodes (`free_move`/`free_rot`/`structure_move`/`structure_rot`), `atom_replace`, and `atom_cut` each carry a **second `diff` output pin** (pin 1) alongside `result` (pin 0) — same two-pin shape as `atom_edit` (`output_pins = [same_as_input("result", <input pin name>), fixed("diff", Molecule)]`; input pin name is `"molecule"` for relax/atom_replace/atom_cut, `"input"` for the movement nodes). The primitive is **`crystolecule::atomic_structure_diff::extract_diff(before, after, ε)`** — derives an applyable `Molecule` diff from a before/after pair **by atom id** (all these nodes mutate a clone in place, so ids are stable — see the doc's §1.5 id-stability audit). Pattern: snapshot `before` *before* the mutation, run the existing mutation, `extract_diff`, return `EvalOutput::multi([result, diff_pin])`; **every** error/early-return path returns two-pin errors `multi([err.clone(), err])` (do NOT copy `atom_edit`'s `EvalOutput::single(error)`, which degrades pin 1 to `None`). `relax`/movement snapshot inline; `atom_replace`/`atom_cut` route through `map_atomic`/`map_atomic_in_region` (which consume the input), so they use the shared helpers in **`evaluator::atom_op`**: `snapshot_atoms` (clone atoms before the map consumes the value), `diff_output_pin`, `eval_output_with_diff`. `relax` additionally has a serde-defaulted `diff_min_move: f64` prune property (ε passed to `extract_diff`; the ONLY FRB regen in the feature — `get/set_relax_data`); the others always use ε = 0.0. Movement/atom_replace/atom_cut Blueprint or non-atomic inputs → **empty diff** (§2.3); movement diffs capture **atom motion only** (§2.4). NO `.cnnd` migration, NO snapshot churn (output pins live on `NodeType`); do NOT override `default_display_all_output_pins` (a diff draws viewport geometry, so keep the pin-0-only default). Deferred (explicitly out of scope): `passivate`/`remove_hydrogen`/`infer_bonds`/`freeze`/`unfreeze`/`atom_union`.
