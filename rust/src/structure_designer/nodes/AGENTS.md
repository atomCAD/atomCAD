...
- **Math/Programming:** `expr`, `value`, `map`, `range`, `parameter`, `sequence`, `array_at`, `array_len`, `array_concat`, `imat3_rows`, `imat3_cols`, `imat3_diag`, `mat3_rows`, `mat3_cols`, `mat3_diag` (3x3 matrix constructors; `_rows`/`_cols` take three vectors, `_diag` takes one). Stored matrix defaults to identity. Wired input pin overrides the corresponding row/column/diagonal at eval. See `doc/design_matrix_types.md`.
