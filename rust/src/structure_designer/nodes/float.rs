...
impl NodeData for FloatData {
    ...
    fn eval<'a>(
        ...
        EvalOutput::single(NetworkResult::Float(self.value))
    }
    ...
    fn get_text_properties(&self) -> Vec<(String, TextValue)> {
        vec![("value".to_string(), TextValue::Float(self.value))]
    }
    ...
    fn set_text_properties(&mut self, props: &HashMap<String, TextValue>) -> Result<(), String> {
        if let Some(v) = props.get("value") {
            self.value = v
                .as_float()
                .ok_or_else(|| "value must be a float".to_string())?;
        }
        Ok(())
    }
}
...
pub fn get_node_type() -> NodeType {
    NodeType {
        ...
        parameters: vec![],
        output_pins: OutputPinDefinition::single(DataType::Float),
        public: true,
        node_data_creator: || Box::new(FloatData { value: 0.0 }),
