...
Edits *inside* an HOF body (`Node.zone`) are addressed by a `scope_path: Vec<u64>` (chain of HOF node ids, `[parent.., hof_id]`). Commands that touch a body carry that path and resolve through `UndoContext::network_in_scope_mut(name, scope_path)` (walks `Node::zone_mut` down the chain). `SetNodeDataCommand`, `SetCollapseModeCommand`, `SetZoneSizeCommand`, `MoveNodesCommand`, `SetFunctionPinRoleCommand`, `SetNodeDisplayCommand`, and `SetOutputPinDisplayCommand` all carry a `scope_path`.

The two **display** commands are the only ones whose `refresh_mode()` varies by scope: `Lightweight` for a top-level toggle (unchanged legacy behavior) but **`Full`** for a body toggle, because adding/removing a *scoped* scene entry is inexpressible in either `Lightweight` (no re-evaluation at all) or `NodeDataChanged` (bare `u64` ids ⇒ top-level only), and `Full` re-derives the eligibility-gated displayed set from scratch. See `doc/design_zero_ary_closure_body_display.md` §5.
