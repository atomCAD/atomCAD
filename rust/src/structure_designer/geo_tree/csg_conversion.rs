...
impl GeoNode {
  ...
  pub fn to_csg_sketch(&self) -> Option<CSGSketch> {
    match &self.kind {
      ...
      GeoNodeKind::Difference2D { base, sub } => {
        Self::difference_2d_to_csg(base.as_ref(), sub.as_ref())
      }
      _ => None
    }    
  }
  ...
  fn internal_to_csg_mesh(&self, is_root: bool) -> Option<CSGMesh> {
    match &self.kind {
      ...
      GeoNodeKind::Extrude { height, direction, shape } => {
        Self::extrude_to_csg(*height, *direction, shape.as_ref())
      }
      GeoNodeKind::Transform { transform, shape } => {
        Self::transform_to_csg(transform, shape.as_ref())
      }
      ...
      GeoNodeKind::Difference3D { base, sub } => {
        Self::difference_3d_to_csg(base.as_ref(), sub.as_ref())
      }
      _ => None
    }
  }
  ...
  fn extrude_to_csg(height: f64, direction: DVec3, shape: &GeoNode) -> Option<CSGMesh> {
  ...
  fn transform_to_csg(transform: &Transform, shape: &GeoNode) -> Option<CSGMesh> {
  ...
  fn union_2d_to_csg(shapes: &Vec<Arc<GeoNode>>) -> Option<CSGSketch> {
    if shapes.is_empty() {
  ...
  fn intersection_2d_to_csg(shapes: &Vec<Arc<GeoNode>>) -> Option<CSGSketch> {
    if shapes.is_empty() {
      return Some(CSGSketch::new());
    }
  ...
  fn difference_2d_to_csg(base: &GeoNode, sub: &GeoNode) -> Option<CSGSketch> {
    let base_csg = base.to_csg_sketch()?;
    let sub_csg = sub.to_csg_sketch()?;
    Some(base_csg.difference(&sub_csg))
  }

  fn union_3d_to_csg(shapes: &Vec<Arc<GeoNode>>) -> Option<CSGMesh> {
    if shapes.is_empty() {
  ...
  fn intersection_3d_to_csg(shapes: &Vec<Arc<GeoNode>>) -> Option<CSGMesh> {
    if shapes.is_empty() {
      return Some(CSGMesh::new());
    }
  ...
  fn difference_3d_to_csg(base: &GeoNode, sub: &GeoNode) -> Option<CSGMesh> {
