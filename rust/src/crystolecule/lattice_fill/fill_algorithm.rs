...
pub fn fill_lattice(
    ...
    let mut batched_evaluator =
        BatchedImplicitEvaluator::new_with_threading(&config.geometry, true);
...

    {
        ...
        for (i, &sdf_value) in sdf_results.iter().enumerate() {
            ...
            if sdf_value <= CRYSTAL_SAMPLE_THRESHOLD {
                let atom_id =
                    atomic_structure.add_atom(atom_data.atomic_number, atom_data.position);
    ...
    LatticeFillResult { atomic_structure }
}
...
fn calculate_motif_space_box(
    ...
    let motif_size = IVec3::new(max_x - min_x + 1, max_y - min_y + 1, max_z - min_z + 1);
...
fn calculate_unit_cell_aabb(cell_real_pos: &DVec3, unit_cell: &UnitCellStruct) -> DAABox {
...
fn do_fill_box(
    ...
    let (motif_min, motif_size) =
        calculate_motif_space_box(unit_cell, motif_offset, &box_to_fill.min, &box_size);
    ...
    for i in 0..motif_size.x {
        for j in 0..motif_size.y {
            for k in 0..motif_size.z {
                ...
                let motif_pos_dvec3 =
                    DVec3::new(motif_pos.x as f64, motif_pos.y as f64, motif_pos.z as f64);
