...
pub struct APIArrayElement {
    ...
    pub fields: Vec<APILiteralField>,
...
/// The name of the single row a simple-typed `array` element exposes. The
/// element-literal setters take an index, not a field name, so this is purely
/// the row's display label.
pub const ARRAY_ELEMENT_VALUE_ROW: &str = "value";
