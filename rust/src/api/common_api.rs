...
pub fn provide_texture(texture_ptr: u64) -> f64 {
    ...
    unsafe {
        ...
        if with_mut_cad_instance(|cad_instance| {
            let background_color = &cad_instance
                .structure_designer
                .preferences
...
pub fn move_camera(eye: APIVec3, target: APIVec3, up: APIVec3) {
    unsafe {
        with_mut_cad_instance(|cad_instance| {
            cad_instance.renderer.move_camera(
                &from_api_vec3(&eye),
                &from_api_vec3(&target),
                &from_api_vec3(&up),
            );
...
pub fn gadget_hit_test(ray_origin: APIVec3, ray_direction: APIVec3) -> Option<i32> {
    unsafe {
        with_cad_instance(|instance| {
            ...
            instance
                .structure_designer
                .gadget_hit_test(origin_vec, direction_vec)
...
pub fn gadget_start_drag(handle_index: i32, ray_origin: APIVec3, ray_direction: APIVec3) {
    unsafe {
        with_mut_cad_instance(|cad_instance| {
            ...
            cad_instance.structure_designer.gadget_start_drag(
                handle_index,
                origin_vec,
                direction_vec,
            );
...
pub fn gadget_drag(handle_index: i32, ray_origin: APIVec3, ray_direction: APIVec3) {
    unsafe {
        with_mut_cad_instance(|instance| {
            ...
            instance
                .structure_designer
                .gadget_drag(handle_index, origin_vec, direction_vec);
...
pub fn sync_gadget_data() -> bool {
    unsafe {
        with_mut_cad_instance_or(
            |instance| instance.structure_designer.sync_gadget_data(),
    ...
    unsafe {
        with_mut_cad_instance(|cad_instance| {
            ...
            if hit_distance.is_none() {
                ...
                let xy_dist_from_ray = if ray_can_hit_xy {
...
pub fn get_camera_canonical_view() -> APICameraCanonicalView {
    unsafe {
        with_cad_instance_or(
            |cad_instance| {
                to_api_camera_canonical_view(cad_instance.renderer.camera.get_canonical_view())
            },
...
pub fn set_camera_canonical_view(view: APICameraCanonicalView) {
    unsafe {
        with_mut_cad_instance(|cad_instance| {
            cad_instance
                .renderer
                .set_camera_canonical_view(to_renderer_camera_canonical_view(view));
            sync_camera_to_active_network(cad_instance);
            refresh_structure_designer_auto(cad_instance);
        });
    }
...
pub fn get_all_elements() -> Vec<ElementSummary> {
    ...
    let mut elements: Vec<ElementSummary> = ATOM_INFO
        .values()
