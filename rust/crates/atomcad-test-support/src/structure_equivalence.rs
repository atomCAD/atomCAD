...
//!
//! The `crystolecule` and `structure_designer` test binaries cannot import from
//! each other — they were separate binaries before the crate split and are now
//! in separate *packages* — so this helper lives in a crate both dev-depend on
...
use atomcad_crystolecule::atomic_structure::atom::DURABLE_FLAGS_MASK;
