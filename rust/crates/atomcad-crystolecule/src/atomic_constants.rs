use glam::f32::Vec3;
use lazy_static::lazy_static;
use std::collections::HashMap;

// Debug visualization atomic numbers - custom elements with carbon radii for consistent visualization
pub const DEBUG_CARBON_GRAY: i32 = 1000;
pub const DEBUG_CARBON_RED: i32 = 1001;
pub const DEBUG_CARBON_GREEN: i32 = 1002;
pub const DEBUG_CARBON_BLUE: i32 = 1003;
pub const DEBUG_CARBON_YELLOW: i32 = 1004;
pub const DEBUG_CARBON_MAGENTA: i32 = 1005;
pub const DEBUG_CARBON_CYAN: i32 = 1006;
pub const DEBUG_CARBON_ORANGE: i32 = 1007;
pub const DEBUG_CARBON_PURPLE: i32 = 1008;
pub const DEBUG_CARBON_TEAL: i32 = 1009;

#[derive(Clone)]
pub struct AtomInfo {
    pub symbol: String,
    pub atomic_number: i32,
    pub element_name: String,
    pub covalent_radius: f64,
    pub van_der_waals_radius: f64,
    pub color: Vec3,
}

lazy_static! {
    /// HashMap containing chemical elements (as uppercase symbols) and their atomic numbers
    pub static ref CHEMICAL_ELEMENTS: HashMap<String, i32> = {
        let mut map = HashMap::new();
        for atom_info in get_all_elements() {
            map.insert(atom_info.symbol.clone(), atom_info.atomic_number);
        }
        map
    };

    pub static ref DEFAULT_ATOM_INFO: AtomInfo = AtomInfo {
        symbol: "X".to_string(),
        atomic_number: 0,
        element_name: "Unknown".to_string(),
        covalent_radius: 0.7,
        van_der_waals_radius: 2.0,  // Default Van der Waals radius for unknown atoms
        color: Vec3::new(0.5, 0.5, 0.5)  // Default gray for unknown atoms
    };

    /// HashMap containing atomic numbers and their corresponding AtomInfo
    /// Source: https://periodictable.com/Properties/A/CovalentRadius.v.log.html
    pub static ref ATOM_INFO: HashMap<i32, AtomInfo> = {
        let mut map = HashMap::new();
        for atom_info in get_all_elements() {
            map.insert(atom_info.atomic_number, atom_info);
        }
        map
    };
}

/// Creates a new AtomInfo with all the necessary information
pub fn create_atom_info(
    atomic_number: i32,
    symbol: &str,
    element_name: &str,
    covalent_radius: f64,
    van_der_waals_radius: f64,
    color: Vec3,
) -> AtomInfo {
    AtomInfo {
        symbol: symbol.to_string(),
        atomic_number,
        element_name: element_name.to_string(),
        covalent_radius,
        van_der_waals_radius,
        color,
    }
}

// ============================================================================
// Passivation (see doc/design_halogen_passivation.md)
// ============================================================================

/// Atomic numbers permitted as passivation terminators: hydrogen plus the four
/// common halogens F, Cl, Br, I. This is the D1 **validation** set and
/// deliberately **includes H**. Contrast [`halogen_bond_length`], which covers
/// the halogens only — call sites branch on `passivant == 1` first and never
/// pass H to it.
pub const ALLOWED_PASSIVANTS: [i16; 5] = [1, 9, 17, 35, 53];

/// Returns `true` if `z` is an allowed passivation element (H or a halogen).
pub fn is_allowed_passivant(z: i16) -> bool {
    ALLOWED_PASSIVANTS.contains(&z)
}

/// Equilibrium single-bond length host–halogen in Å (molecular values).
/// `halogen` ∈ {F (9), Cl (17), Br (35), I (53)}; hydrogen is handled by each
/// call site's existing per-context path (see doc/design_halogen_passivation.md,
/// D2), so H must never reach this function.
///
/// Falls back to the covalent-radii sum for host/halogen pairs not in the table
/// (the `—` cells of the D2 table, and any non-halogen `halogen` argument).
///
/// Values are rounded molecular experimental bond lengths, same provenance style
/// as `XH_BOND_LENGTHS` (Calculla / Wikipedia / NIST CCCBDB).
pub fn halogen_bond_length(host: i16, halogen: i16) -> f64 {
    // Columns are [F, Cl, Br, I]; `None` = no tabulated value → radii-sum fallback.
    const TABLE: &[(i16, [Option<f64>; 4])] = &[
        //                 F           Cl          Br          I
        (6, [Some(1.35), Some(1.77), Some(1.94), Some(2.14)]), // C
        (14, [Some(1.60), Some(2.02), Some(2.16), Some(2.44)]), // Si
        (32, [Some(1.70), Some(2.10), Some(2.30), Some(2.51)]), // Ge
        (5, [Some(1.31), Some(1.75), Some(1.87), Some(2.10)]), // B
        (7, [Some(1.36), Some(1.75), None, None]),             // N
        (8, [Some(1.42), Some(1.70), None, None]),             // O
        (15, [Some(1.57), Some(2.03), Some(2.20), None]),      // P
        (16, [Some(1.56), Some(2.05), None, None]),            // S
    ];

    let col = match halogen {
        9 => Some(0),
        17 => Some(1),
        35 => Some(2),
        53 => Some(3),
        _ => None, // not a halogen — fall through to radii sum
    };

    if let Some(col) = col {
        for &(z, cols) in TABLE {
            if z == host {
                if let Some(len) = cols[col] {
                    return len;
                }
                break; // host is tabulated but this column is `—`; use fallback
            }
        }
    }

    // Fallback: sum of covalent radii.
    let r_host = ATOM_INFO
        .get(&(host as i32))
        .unwrap_or(&DEFAULT_ATOM_INFO)
        .covalent_radius;
    let r_halogen = ATOM_INFO
        .get(&(halogen as i32))
        .unwrap_or(&DEFAULT_ATOM_INFO)
        .covalent_radius;
    r_host + r_halogen
}

/// Contains the registry of all chemical elements in the system
fn get_all_elements() -> Vec<AtomInfo> {
    vec![
        // Elements are ordered by atomic number
        create_atom_info(1, "H", "Hydrogen", 0.31, 1.20, Vec3::new(1.0, 1.0, 1.0)),
        create_atom_info(2, "He", "Helium", 0.28, 1.43, Vec3::new(0.85, 1.00, 1.00)),
        create_atom_info(3, "Li", "Lithium", 1.28, 2.12, Vec3::new(0.80, 0.50, 1.00)),
        create_atom_info(
            4,
            "Be",
            "Beryllium",
            0.96,
            1.98,
            Vec3::new(0.76, 1.00, 0.00),
        ),
        create_atom_info(5, "B", "Boron", 0.85, 1.91, Vec3::new(1.00, 0.71, 0.71)),
        create_atom_info(6, "C", "Carbon", 0.76, 1.77, Vec3::new(0.18, 0.18, 0.18)),
        create_atom_info(
            7,
            "N",
            "Nitrogen",
            0.71,
            1.66,
            Vec3::new(0.187, 0.375, 0.97),
        ),
        create_atom_info(8, "O", "Oxygen", 0.66, 1.50, Vec3::new(1.0, 0.05, 0.05)),
        create_atom_info(9, "F", "Fluorine", 0.57, 1.46, Vec3::new(0.56, 0.88, 0.31)),
        create_atom_info(10, "Ne", "Neon", 0.58, 1.58, Vec3::new(0.70, 0.89, 0.96)),
        create_atom_info(11, "Na", "Sodium", 1.66, 2.50, Vec3::new(0.67, 0.36, 0.95)),
        create_atom_info(
            12,
            "Mg",
            "Magnesium",
            1.41,
            2.51,
            Vec3::new(0.54, 1.00, 0.00),
        ),
        create_atom_info(
            13,
            "Al",
            "Aluminium",
            1.21,
            2.25,
            Vec3::new(0.75, 0.65, 0.65),
        ),
        create_atom_info(14, "Si", "Silicon", 1.11, 2.19, Vec3::new(0.94, 0.78, 0.63)),
        create_atom_info(
            15,
            "P",
            "Phosphorus",
            1.07,
            1.90,
            Vec3::new(1.00, 0.50, 0.00),
        ),
        create_atom_info(16, "S", "Sulfur", 1.05, 1.89, Vec3::new(1.00, 1.00, 0.19)),
        create_atom_info(
            17,
            "Cl",
            "Chlorine",
            1.02,
            1.82,
            Vec3::new(0.12, 0.94, 0.12),
        ),
        create_atom_info(18, "Ar", "Argon", 1.06, 1.83, Vec3::new(0.50, 0.82, 0.89)),
        create_atom_info(
            19,
            "K",
            "Potassium",
            2.03,
            2.73,
            Vec3::new(0.56, 0.25, 0.83),
        ),
        create_atom_info(20, "Ca", "Calcium", 1.76, 2.62, Vec3::new(0.24, 1.00, 0.00)),
        create_atom_info(21, "Sc", "Scandium", 1.7, 2.58, Vec3::new(0.90, 0.90, 0.90)),
        create_atom_info(22, "Ti", "Titanium", 1.6, 2.46, Vec3::new(0.75, 0.76, 0.78)),
        create_atom_info(23, "V", "Vanadium", 1.53, 2.42, Vec3::new(0.65, 0.65, 0.67)),
        create_atom_info(
            24,
            "Cr",
            "Chromium",
            1.39,
            2.45,
            Vec3::new(0.54, 0.60, 0.78),
        ),
        create_atom_info(
            25,
            "Mn",
            "Manganese",
            1.39,
            2.45,
            Vec3::new(0.61, 0.48, 0.78),
        ),
        create_atom_info(26, "Fe", "Iron", 1.32, 2.44, Vec3::new(0.88, 0.40, 0.20)),
        create_atom_info(27, "Co", "Cobalt", 1.26, 2.40, Vec3::new(0.94, 0.56, 0.63)),
        create_atom_info(28, "Ni", "Nickel", 1.24, 2.40, Vec3::new(0.31, 0.82, 0.31)),
        create_atom_info(29, "Cu", "Copper", 1.32, 2.38, Vec3::new(0.78, 0.50, 0.20)),
        create_atom_info(30, "Zn", "Zinc", 1.22, 2.39, Vec3::new(0.49, 0.50, 0.69)),
        create_atom_info(31, "Ga", "Gallium", 1.22, 2.32, Vec3::new(0.76, 0.56, 0.56)),
        create_atom_info(32, "Ge", "Germanium", 1.2, 2.29, Vec3::new(0.4, 0.56, 0.56)),
        create_atom_info(33, "As", "Arsenic", 1.19, 1.88, Vec3::new(0.74, 0.50, 0.89)),
        create_atom_info(34, "Se", "Selenium", 1.2, 1.82, Vec3::new(1.00, 0.63, 0.00)),
        create_atom_info(35, "Br", "Bromine", 1.2, 1.86, Vec3::new(0.65, 0.16, 0.16)),
        create_atom_info(36, "Kr", "Krypton", 1.16, 2.25, Vec3::new(0.36, 0.72, 0.82)),
        create_atom_info(37, "Rb", "Rubidium", 2.2, 3.21, Vec3::new(0.44, 0.18, 0.69)),
        create_atom_info(
            38,
            "Sr",
            "Strontium",
            1.95,
            2.84,
            Vec3::new(0.00, 1.00, 0.00),
        ),
        create_atom_info(39, "Y", "Yttrium", 1.9, 2.75, Vec3::new(0.58, 1.00, 1.00)),
        create_atom_info(
            40,
            "Zr",
            "Zirconium",
            1.75,
            2.52,
            Vec3::new(0.58, 0.88, 0.88),
        ),
        create_atom_info(41, "Nb", "Niobium", 1.64, 2.56, Vec3::new(0.45, 0.76, 0.79)),
        create_atom_info(
            42,
            "Mo",
            "Molybdenum",
            1.54,
            2.45,
            Vec3::new(0.33, 0.71, 0.71),
        ),
        create_atom_info(
            43,
            "Tc",
            "Technetium",
            1.47,
            2.44,
            Vec3::new(0.23, 0.62, 0.62),
        ),
        create_atom_info(
            44,
            "Ru",
            "Ruthenium",
            1.46,
            2.46,
            Vec3::new(0.14, 0.56, 0.56),
        ),
        create_atom_info(45, "Rh", "Rhodium", 1.42, 2.44, Vec3::new(0.04, 0.49, 0.55)),
        create_atom_info(
            46,
            "Pd",
            "Palladium",
            1.39,
            2.15,
            Vec3::new(0.00, 0.41, 0.52),
        ),
        create_atom_info(47, "Ag", "Silver", 1.45, 2.53, Vec3::new(0.75, 0.75, 0.75)),
        create_atom_info(48, "Cd", "Cadmium", 1.44, 2.49, Vec3::new(1.00, 0.85, 0.56)),
        create_atom_info(49, "In", "Indium", 1.42, 2.43, Vec3::new(0.65, 0.46, 0.45)),
        create_atom_info(50, "Sn", "Tin", 1.39, 2.42, Vec3::new(0.40, 0.50, 0.50)),
        create_atom_info(
            51,
            "Sb",
            "Antimony",
            1.39,
            2.47,
            Vec3::new(0.62, 0.39, 0.71),
        ),
        create_atom_info(
            52,
            "Te",
            "Tellurium",
            1.38,
            1.99,
            Vec3::new(0.83, 0.48, 0.00),
        ),
        create_atom_info(53, "I", "Iodine", 1.39, 2.04, Vec3::new(0.58, 0.00, 0.58)),
        create_atom_info(54, "Xe", "Xenon", 1.4, 2.06, Vec3::new(0.26, 0.62, 0.69)),
        create_atom_info(55, "Cs", "Cesium", 2.44, 3.48, Vec3::new(0.34, 0.09, 0.56)),
        create_atom_info(56, "Ba", "Barium", 2.15, 3.03, Vec3::new(0.00, 0.79, 0.00)),
        create_atom_info(
            57,
            "La",
            "Lanthanum",
            2.07,
            2.98,
            Vec3::new(0.44, 0.83, 1.00),
        ),
        create_atom_info(58, "Ce", "Cerium", 2.04, 2.88, Vec3::new(1.00, 1.00, 0.78)),
        create_atom_info(
            59,
            "Pr",
            "Praseodymium",
            2.03,
            2.92,
            Vec3::new(0.85, 1.00, 0.78),
        ),
        create_atom_info(
            60,
            "Nd",
            "Neodymium",
            2.01,
            2.95,
            Vec3::new(0.78, 1.00, 0.78),
        ),
        create_atom_info(
            61,
            "Pm",
            "Promethium",
            1.99,
            2.93,
            Vec3::new(0.64, 1.00, 0.78),
        ),
        create_atom_info(
            62,
            "Sm",
            "Samarium",
            1.98,
            2.90,
            Vec3::new(0.56, 1.00, 0.78),
        ),
        create_atom_info(
            63,
            "Eu",
            "Europium",
            1.98,
            2.87,
            Vec3::new(0.38, 1.00, 0.78),
        ),
        create_atom_info(
            64,
            "Gd",
            "Gadolinium",
            1.96,
            2.83,
            Vec3::new(0.27, 1.00, 0.78),
        ),
        create_atom_info(65, "Tb", "Terbium", 1.94, 2.79, Vec3::new(0.19, 1.00, 0.78)),
        create_atom_info(
            66,
            "Dy",
            "Dysprosium",
            1.92,
            2.87,
            Vec3::new(0.12, 1.00, 0.78),
        ),
        create_atom_info(67, "Ho", "Holmium", 1.92, 2.81, Vec3::new(0.00, 1.00, 0.61)),
        create_atom_info(68, "Er", "Erbium", 1.89, 2.83, Vec3::new(0.00, 0.90, 0.46)),
        create_atom_info(69, "Tm", "Thulium", 1.9, 2.79, Vec3::new(0.00, 0.83, 0.32)),
        create_atom_info(
            70,
            "Yb",
            "Ytterbium",
            1.87,
            2.80,
            Vec3::new(0.00, 0.75, 0.22),
        ),
        create_atom_info(
            71,
            "Lu",
            "Lutetium",
            1.87,
            2.74,
            Vec3::new(0.00, 0.67, 0.14),
        ),
        create_atom_info(72, "Hf", "Hafnium", 1.75, 2.63, Vec3::new(0.30, 0.76, 1.00)),
        create_atom_info(73, "Ta", "Tantalum", 1.7, 2.53, Vec3::new(0.30, 0.65, 1.00)),
        create_atom_info(74, "W", "Tungsten", 1.62, 2.57, Vec3::new(0.13, 0.58, 0.84)),
        create_atom_info(75, "Re", "Rhenium", 1.51, 2.49, Vec3::new(0.15, 0.49, 0.67)),
        create_atom_info(76, "Os", "Osmium", 1.44, 2.48, Vec3::new(0.15, 0.40, 0.59)),
        create_atom_info(77, "Ir", "Iridium", 1.41, 2.41, Vec3::new(0.09, 0.33, 0.53)),
        create_atom_info(
            78,
            "Pt",
            "Platinum",
            1.36,
            2.29,
            Vec3::new(0.82, 0.82, 0.88),
        ),
        create_atom_info(79, "Au", "Gold", 1.36, 2.32, Vec3::new(1.00, 0.82, 0.14)),
        create_atom_info(80, "Hg", "Mercury", 1.32, 2.45, Vec3::new(0.72, 0.72, 0.82)),
        create_atom_info(
            81,
            "Tl",
            "Thallium",
            1.45,
            2.47,
            Vec3::new(0.65, 0.33, 0.30),
        ),
        create_atom_info(82, "Pb", "Lead", 1.46, 2.60, Vec3::new(0.34, 0.35, 0.38)),
        create_atom_info(83, "Bi", "Bismuth", 1.48, 2.54, Vec3::new(0.62, 0.31, 0.71)),
        create_atom_info(84, "Po", "Polonium", 1.4, 2.3, Vec3::new(0.67, 0.36, 0.00)),
        create_atom_info(85, "At", "Astatine", 1.5, 2.2, Vec3::new(0.46, 0.31, 0.27)),
        create_atom_info(86, "Rn", "Radon", 1.5, 2.4, Vec3::new(0.26, 0.51, 0.59)),
        create_atom_info(87, "Fr", "Francium", 2.6, 3.7, Vec3::new(0.26, 0.00, 0.40)),
        create_atom_info(88, "Ra", "Radium", 2.21, 3.2, Vec3::new(0.00, 0.49, 0.00)),
        create_atom_info(
            89,
            "Ac",
            "Actinium",
            2.15,
            2.80,
            Vec3::new(0.44, 0.67, 0.98),
        ),
        create_atom_info(90, "Th", "Thorium", 2.06, 2.93, Vec3::new(0.00, 0.73, 1.00)),
        create_atom_info(
            91,
            "Pa",
            "Protactinium",
            2.0,
            2.88,
            Vec3::new(0.00, 0.63, 1.00),
        ),
        create_atom_info(92, "U", "Uranium", 1.96, 2.71, Vec3::new(0.00, 0.56, 1.00)),
        create_atom_info(
            93,
            "Np",
            "Neptunium",
            1.9,
            2.82,
            Vec3::new(0.00, 0.50, 1.00),
        ),
        create_atom_info(
            94,
            "Pu",
            "Plutonium",
            1.87,
            2.81,
            Vec3::new(0.00, 0.42, 1.00),
        ),
        create_atom_info(
            95,
            "Am",
            "Americium",
            1.8,
            2.83,
            Vec3::new(0.33, 0.36, 0.95),
        ),
        create_atom_info(96, "Cm", "Curium", 1.69, 3.05, Vec3::new(0.47, 0.36, 0.89)),
        // Debug visualization elements (1000-1009) - all have carbon radii for consistent visualization
        create_atom_info(
            DEBUG_CARBON_GRAY,
            "DCGray",
            "DebugCarbonGray",
            0.76,
            1.77,
            Vec3::new(0.18, 0.18, 0.18),
        ),
        create_atom_info(
            DEBUG_CARBON_RED,
            "DCRed",
            "DebugCarbonRed",
            0.76,
            1.77,
            Vec3::new(1.0, 0.0, 0.0),
        ),
        create_atom_info(
            DEBUG_CARBON_GREEN,
            "DCGreen",
            "DebugCarbonGreen",
            0.76,
            1.77,
            Vec3::new(0.0, 1.0, 0.0),
        ),
        create_atom_info(
            DEBUG_CARBON_BLUE,
            "DCBlue",
            "DebugCarbonBlue",
            0.76,
            1.77,
            Vec3::new(0.0, 0.0, 1.0),
        ),
        create_atom_info(
            DEBUG_CARBON_YELLOW,
            "DCYellow",
            "DebugCarbonYellow",
            0.76,
            1.77,
            Vec3::new(1.0, 1.0, 0.0),
        ),
        create_atom_info(
            DEBUG_CARBON_MAGENTA,
            "DCMagenta",
            "DebugCarbonMagenta",
            0.76,
            1.77,
            Vec3::new(1.0, 0.0, 1.0),
        ),
        create_atom_info(
            DEBUG_CARBON_CYAN,
            "DCCyan",
            "DebugCarbonCyan",
            0.76,
            1.77,
            Vec3::new(0.0, 1.0, 1.0),
        ),
        create_atom_info(
            DEBUG_CARBON_ORANGE,
            "DCOrange",
            "DebugCarbonOrange",
            0.76,
            1.77,
            Vec3::new(1.0, 0.5, 0.0),
        ),
        create_atom_info(
            DEBUG_CARBON_PURPLE,
            "DCPurple",
            "DebugCarbonPurple",
            0.76,
            1.77,
            Vec3::new(0.5, 0.0, 1.0),
        ),
        create_atom_info(
            DEBUG_CARBON_TEAL,
            "DCTeal",
            "DebugCarbonTeal",
            0.76,
            1.77,
            Vec3::new(0.0, 0.8, 0.5),
        ),
    ]
}

// =============================================================================
// Parameter element constants (motif_edit mode)
// =============================================================================
//
// These reserved *negative* atomic numbers are a property of the atomic-number
// encoding itself, so they belong beside the element table rather than in the
// node that happens to author them. `display` reads them per atom while
// tessellating, which is why they moved down here in Phase 5 of
// `doc/design_rust_crate_split.md` (D7): the alternative was
// `atomcad-display` depending on `structure_designer`.
//
// D7 names only `param_atomic_number_to_index`. The whole five-item family
// travels with it because the other four are the same encoding read from the
// other end — they are defined in terms of `PARAM_ELEMENT_BASE` /
// `MAX_PARAM_ELEMENTS`, which have to move for the named function to compile
// at all, and splitting an inverse pair across a crate boundary buys nothing.

/// First reserved atomic number for parameter elements.
/// PARAM_1 = -100, PARAM_2 = -101, etc.
pub const PARAM_ELEMENT_BASE: i16 = -100;

/// Maximum number of parameter elements supported.
pub const MAX_PARAM_ELEMENTS: usize = 100; // -100 to -199

/// Convert an internal parameter atomic number (-100, -101, ...)
/// to a motif parameter index (0, 1, ...).
pub fn param_atomic_number_to_index(atomic_number: i16) -> Option<usize> {
    if atomic_number <= PARAM_ELEMENT_BASE
        && atomic_number > PARAM_ELEMENT_BASE - MAX_PARAM_ELEMENTS as i16
    {
        Some((PARAM_ELEMENT_BASE - atomic_number) as usize)
    } else {
        None
    }
}

/// Convert a motif parameter index (0, 1, ...) to an internal
/// reserved atomic number (-100, -101, ...).
pub fn param_index_to_atomic_number(index: usize) -> i16 {
    PARAM_ELEMENT_BASE - index as i16
}

/// Convert an internal reserved atomic number to the motif's
/// negative atomic number convention (-1, -2, ...).
pub fn param_atomic_number_to_motif(atomic_number: i16) -> i16 {
    -(param_atomic_number_to_index(atomic_number).unwrap() as i16 + 1)
}

/// Returns true if the atomic number is a parameter element.
pub fn is_param_element(atomic_number: i16) -> bool {
    param_atomic_number_to_index(atomic_number).is_some()
}
