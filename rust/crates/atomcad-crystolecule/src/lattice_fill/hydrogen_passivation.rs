...
use crate::atomic_constants::{ATOM_INFO, halogen_bond_length};
use crate::atomic_structure::AtomicStructure;
use crate::motif::SiteSpecifier;
use glam::f64::DVec3;
...
pub fn hydrogen_passivate(
    ...
    for (address, atom_id) in atom_tracker.iter_atoms() {
        ...
        let (atom_position, atom_atomic_number, actual_bond_count) =
            match atomic_structure.get_atom(atom_id) {
        ...
        for &bond_index in &config.motif.bonds_by_site2_index[site_index] {
            ...
            let atom_id_1 =
                atom_tracker.get_atom_id_for_specifier(second_site_base_pos, &bond.site_1);
...
fn hydrogen_passivate_dangling_bond(
    ...
    let found_site_pos =
        config.motif.sites[found_site.site_index].position + found_site.relative_cell.as_dvec3();
