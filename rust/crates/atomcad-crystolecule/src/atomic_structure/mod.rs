...
use crate::visualization::AtomicStructureVisualization;
...
///
/// This is the *authoritative* definition. A same-named, Dart-facing twin lives
/// in the root crate's `api/common_api_types.rs` with `From` impls both ways
/// (`doc/design_rust_crate_split.md`, D6 + D9a) — the twin keeps the name
/// because it is what the generated Dart already declares. Conversion happens
/// at the `api/` boundary; everything below it uses this type.
///
/// It derives `Serialize`/`Deserialize` because `edit_atom`'s `SelectCommand`
/// is persisted in `.cnnd` files. The unit-variant names must not change.
