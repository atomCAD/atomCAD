...
use atomcad_crystolecule::atomic_structure::{AtomRenderStyle, AtomicStructure, BondReference};
use glam::Vec3;
    ...
    for (_, atom) in structure.iter_atoms() {
        for bond in &atom.bonds {
            ...
            if let Some(other_atom) = structure.get_atom(other_id) {
                let has_reverse_bond = other_atom
                    .bonds
                    .iter()
...
fn test_iter_atoms() {
    ...
    let atomic_numbers: Vec<i16> = structure
        .iter_atoms()
...
fn test_add_bond_prevents_duplicates() {
    ...
    let atom1 = structure.get_atom(id1).unwrap();
    let bonds_to_atom2 = atom1
        .bonds
        .iter()
        .filter(|b| b.other_atom_id() == id2)
        .count();
...
fn test_bond_reference_equality() {
    ...
    let ref2 = BondReference {
        atom_id1: 2,
        atom_id2: 1,
    };
    let ref3 = BondReference {
        atom_id1: 1,
        atom_id2: 3,
    };
    ...
    assert_eq!(
        ref1, ref2,
        "Bond references should be equal regardless of order"
    );
...
fn test_get_num_of_bonds_no_duplicates() {
    ...
    assert_eq!(
        ...
        2,
        "Should count each bond only once"
    );
}
...
fn test_rebuild_after_deletions() {
    ...
    // Delete some atoms
    let ids_to_delete: Vec<u32> = structure.iter_atoms().take(2).map(|(id, _)| *id).collect();
...
fn test_invariant_all_bonds_bidirectional() {
    ...
    assert!(
        verify_bonds_bidirectional(&structure),
        "All bonds must be bidirectional"
    );
}
...
fn test_invariant_grid_matches_positions() {
    let structure = create_test_molecule();
    assert!(
        verify_grid_consistency(&structure),
        "Grid must match atom positions"
    );
}
...
fn test_invariant_no_dangling_bonds() {
    ...
    for (_, atom) in structure.iter_atoms() {
        for bond in &atom.bonds {
            let other_id = bond.other_atom_id();
            assert!(
                structure.get_atom(other_id).is_some(),
                "Bond references atom {} which doesn't exist",
                other_id
            );
        }
    }
...
fn test_deleted_ids_not_reused() {
    ...
    structure.delete_atom(id1);
    assert!(
        structure.get_atom(id1).is_none(),
        "Atom 1 should be deleted"
    );
