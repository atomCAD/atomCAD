...
use atomcad_crystolecule::io::mol_exporter::save_mol_v3000;
use glam::f64::DVec3;
use std::fs;
use std::path::Path;
...
fn test_save_mol_v3000_simple_molecule() {
    ...
    assert!(
        content.contains("Generated by atomCAD"),
    ...
    let bond_lines: Vec<&str> = bond_section
        .lines()
        .filter(|line| {
            line.starts_with("M  V30") && !line.contains("BEGIN") && !line.contains("END")
        })
        .collect();
...
fn test_save_mol_v3000_empty_structure() {
    ...
    assert!(
        ...
        "Empty MOL file was not created"
    );
    ...
    assert!(
        content.contains("M  V30 COUNTS 0 0 0 0 0"),
        "Empty COUNTS line incorrect"
    );
...
fn test_save_mol_v3000_ethane_molecule() {
    ...
    assert!(
        result.is_ok(),
        "Failed to save ethane MOL file: {:?}",
        result.err()
    );
    ...
    assert!(
        content.contains("M  V30 COUNTS 8 7 0 0 0"),
        "Ethane COUNTS line incorrect"
    );
...
fn test_save_mol_v3000_with_double_bond() {
    ...
    assert!(
        result.is_ok(),
        "Failed to save ethylene MOL file: {:?}",
        result.err()
    );
    ...
    // Verify correct atom and bond counts
    assert!(
        content.contains("M  V30 COUNTS 6 5 0 0 0"),
        "Ethylene COUNTS line incorrect"
    );
