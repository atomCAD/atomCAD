use atomcad_crystolecule::io::cif::{CifLoadError, load_cif, load_cif_from_str};
use atomcad_test_support::fixture_path;
use glam::DVec3;

fn cif_fixture(name: &str) -> String {
    fixture_path(&format!("cif/{}", name))
        .to_string_lossy()
        .into_owned()
}
...
fn count_element(
    result: &atomcad_crystolecule::io::cif::CifLoadResult,
    atomic_number: i16,
) -> usize {
    result
        .atoms
...
fn load_diamond_cif_atom_count() {
    let result = load_cif(&cif_fixture("diamond.cif"), None).unwrap();
...
fn load_nacl_cif_atom_count() {
    let result = load_cif(&cif_fixture("nacl.cif"), None).unwrap();
...
fn load_hexagonal_cif_atom_count() {
    let result = load_cif(&cif_fixture("hexagonal.cif"), None).unwrap();
...
fn load_multi_block_first_block_by_default() {
    let result = load_cif(&cif_fixture("multi_block.cif"), None).unwrap();
...
fn load_multi_block_select_by_name() {
    let result = load_cif(&cif_fixture("multi_block.cif"), Some("nacl")).unwrap();
...
fn load_multi_block_unknown_name_error() {
    let err = load_cif(&cif_fixture("multi_block.cif"), Some("nonexistent")).unwrap_err();
