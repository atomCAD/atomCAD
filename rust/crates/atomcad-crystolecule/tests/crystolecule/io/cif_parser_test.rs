use atomcad_crystolecule::io::cif::parser::parse_cif;
...
fn parse_diamond_fixture() {
    let content = std::fs::read_to_string(fixture_path("cif/diamond.cif")).unwrap();
...
fn parse_nacl_fixture() {
    let content = std::fs::read_to_string(fixture_path("cif/nacl.cif")).unwrap();
    let doc = parse_cif(&content).unwrap();
    assert_eq!(doc.data_blocks.len(), 1);
    let block = &doc.data_blocks[0];
...
fn parse_hexagonal_fixture() {
    let content = std::fs::read_to_string(fixture_path("cif/hexagonal.cif")).unwrap();
    let doc = parse_cif(&content).unwrap();
    let block = &doc.data_blocks[0];
...
fn parse_multi_block_fixture() {
    let content = std::fs::read_to_string(fixture_path("cif/multi_block.cif")).unwrap();
    let doc = parse_cif(&content).unwrap();
    assert_eq!(doc.data_blocks.len(), 2);
    assert_eq!(doc.data_blocks[0].name, "diamond");
    assert_eq!(doc.data_blocks[1].name, "nacl");
...
fn parse_with_bonds_fixture() {
    let content = std::fs::read_to_string(fixture_path("cif/with_bonds.cif")).unwrap();
    let doc = parse_cif(&content).unwrap();
    assert_eq!(doc.data_blocks.len(), 1);
    let block = &doc.data_blocks[0];
