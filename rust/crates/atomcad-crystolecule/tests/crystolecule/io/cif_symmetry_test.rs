use atomcad_crystolecule::io::cif::symmetry::{
...
fn expand_diamond_2_to_8_atoms() {
    let diamond_cif = std::fs::read_to_string(fixture_path("cif/diamond.cif")).unwrap();
    let doc = atomcad_crystolecule::io::cif::parser::parse_cif(&diamond_cif).unwrap();
...
fn expand_nacl_2_to_8_atoms() {
    let nacl_cif = std::fs::read_to_string(fixture_path("cif/nacl.cif")).unwrap();
    let doc = atomcad_crystolecule::io::cif::parser::parse_cif(&nacl_cif).unwrap();
    let block = &doc.data_blocks[0];
...
fn expand_wurtzite_zns() {
    let cif = std::fs::read_to_string(fixture_path("cif/hexagonal.cif")).unwrap();
    let doc = atomcad_crystolecule::io::cif::parser::parse_cif(&cif).unwrap();
    let block = &doc.data_blocks[0];
...
fn parse_all_nacl_symmetry_operations() {
    let cif = std::fs::read_to_string(fixture_path("cif/nacl.cif")).unwrap();
    let doc = atomcad_crystolecule::io::cif::parser::parse_cif(&cif).unwrap();
    let block = &doc.data_blocks[0];
