...
//! `copy_content_into` and the boundary splice.
//!
//! Making room for the inlined content is no longer this module's job — the
//! quadrant shift it used to own was retired in favour of
//! `layout::motion::grow_rect` (`doc/design_incremental_layout.md` D6/D11), and
//! its tests moved to `layout_motion_test.rs` along with it.
...
use atomcad_structure_designer::node_data::{EvalOutput, NodeData};
use atomcad_structure_designer::node_inlining::{
    content_bounding_box, copy_content_into, splice_inline_boundary,
};
use atomcad_structure_designer::node_network::{
    Argument, IncomingWire, Node, NodeDisplayType, NodeNetwork, SourcePin,
};
use atomcad_structure_designer::node_network_gadget::NodeNetworkGadget;
use atomcad_structure_designer::node_type::NodeType;
