...
use atomcad_structure_designer::node_network::{Argument, IncomingWire, SourcePin};
...
fn add_expr_to_network(
    network: &mut atomcad_structure_designer::node_network::NodeNetwork,
    expression: &str,
    parameters: Vec<(String, DataType)>,
) -> u64 {
    let expr_params: Vec<ExprParameter> = parameters
...
fn collect_all_errors(
    network: &atomcad_structure_designer::node_network::NodeNetwork,
) -> Vec<String> {
...
fn find_body_error_for_node(
    body: &atomcad_structure_designer::node_network::NodeNetwork,
    node_id: u64,
) -> Option<String> {
