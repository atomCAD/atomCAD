...
use atomcad_structure_designer::layout::{
    LayoutAlgorithm, WireKey, collect_all_wires, layout_incremental, layout_subgraph,
    rendered_node_size,
};
...
use super::layout_test_support::{empty_network, id_by_path, node_by_path, node_by_path_mut};
...
impl Fixture {
    ...
    fn place(&mut self, path: &[&str], x: f64, y: f64) -> &mut Self {
    ...
    fn body_size(&mut self, path: &[&str], width: f64, height: f64) -> &mut Self {
    ...
    fn delta_paths(&self, after: &Drawing) -> HashSet<NamePath> {
...
/// What one pass did.
struct Outcome {
    moved: Vec<(NamePath, DVec2, DVec2)>,
    after: Drawing,
}

impl Outcome {
    fn rect(&self, path: &[&str]) -> Placed {
        self.after
            .get(&p(path))
            .unwrap_or_else(|| panic!("no node at {path:?} after the pass"))
    }

    fn moved_paths(&self) -> Vec<NamePath> {
    ...
    fn assert_only_moved(&self, allowed: &[&[&str]]) {
    ...
    fn shift(&self, path: &[&str]) -> Option<DVec2> {
        self.moved
            .iter()
...
fn p(segments: &[&str]) -> NamePath {
    segments.iter().map(|s| s.to_string()).collect()
}

fn center(rect: Placed) -> DVec2 {
    rect.position + rect.size * 0.5
}

/// One `expr` statement: `name = expr { x: source, … }`.
fn expr(name: &str, source: &str) -> String {
    format!(
