...
use atomcad_structure_designer::invariants::{
...
fn arg_count_mismatch_is_reported() {
    ...
    net_mut(&mut d)
        ...
        .unwrap()
        .arguments
        .push(atomcad_structure_designer::node_network::Argument::new());
...
fn debug_wrapper_panics_on_arg_count_mismatch() {
    use atomcad_structure_designer::invariants::debug_assert_network_invariants;
    let mut d = designer_with_net();
    let id = d.add_node("sphere", DVec2::ZERO);
    d.validate_active_network();
    net_mut(&mut d)
