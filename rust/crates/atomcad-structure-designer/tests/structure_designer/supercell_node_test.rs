...
fn set_node_data<T: atomcad_structure_designer::node_data::NodeData + 'static>(
...
fn subtitle_shows_determinant_when_matrix_unconnected() {
    ...
    let subtitle = atomcad_structure_designer::node_data::NodeData::get_subtitle(&data, &connected);
    assert_eq!(subtitle.as_deref(), Some("det = 8"));

    let singular = SupercellData {
        matrix: [[1, 0, 0], [0, 0, 0], [0, 0, 1]],
    };
    let sub2 = atomcad_structure_designer::node_data::NodeData::get_subtitle(&singular, &connected);
    assert!(sub2.as_deref().unwrap().contains("singular"));

    let handed = SupercellData {
        matrix: [[0, 1, 0], [1, 0, 0], [0, 0, 1]],
    };
    let sub3 = atomcad_structure_designer::node_data::NodeData::get_subtitle(&handed, &connected);
...
fn text_properties_roundtrip_preserves_matrix() {
    ...
    let props: HashMap<String, TextValue> =
        atomcad_structure_designer::node_data::NodeData::get_text_properties(&original)
            .into_iter()
            .collect();
    ...
    atomcad_structure_designer::node_data::NodeData::set_text_properties(&mut restored, &props)
        .unwrap();
