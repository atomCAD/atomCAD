...
- `node_error_origins: HashMap<NodeRef, Vec<ErrorOrigin>>` — **origin links** (`doc/design_error_management.md` D7): per consumer, the address(es) its failing input(s) came from. Recorded at exactly two kinds of site: the wire-resolution choke point in `evaluate_arg` (both the array and single-argument branches, right where the resolved value turns out to be an `Error` — recording *there* covers text-wrapping, verbatim-forwarding and tolerating consumers uniformly) and the two custom-network wrap sites, which do not pass through `evaluate_arg` and record the instance → child-return boundary hop explicitly. An errored node with no link is a **root cause**; the panel lists roots and collapses the rest behind them. Two things to preserve: the link value's `address` is derived from the **network stack** (`wire_source_address` — only the stack distinguishes a zone-body hop from a custom-network hop, which is what makes the address jump-ready), while `source_ref` is derived from `eval_scope_path` so it keys the same way `node_errors` does and a chain walk is a plain map lookup. Consequence worth knowing: a lazy walker's per-element step stands up a body-only stack, so no address resolves there and body nodes under `map`/`filter` record no links (they surface as their own root causes, which is the honest answer anyway).
...
- `zone_closure::build_node_function_closure` — the **function-pin** synthesizer (`doc/design_function_pins.md`): builds a `ZoneClosure` from "the whole node viewed as a function of its inputs" — clones the node into a one-node synthetic body, and per input pin either feeds it from a `ZoneInput` parameter, forwards its wire(s) rebased `+1` as a capture, or leaves the body argument empty so the cloned `NodeData`'s stored-data fallback supplies it. Reached from the revived `output_pin_index == -1` branch in `NetworkEvaluator::evaluate`, so the title-bar `-1` pin produces a `NetworkResult::Function` consumed by the HOF `f` pins / `apply` like any other closure. **Which of the three arms a pin takes is decided by `node_network::function_pin_dispositions`, and the carried `param_types` / `return_type` come from `NodeTypeRegistry::resolve_function_pin_signature`** — the same helpers the `-1` type resolver uses, so the built body and the advertised wire type cannot drift (`doc/design_function_pin_roles.md`). Rejects a node whose pin-0 output type does not *resolve* (note: resolution, not declaration — a `same_as_input` pin-0 resolves fine once a capture or a `Delayed` preview wire witnesses it).
...
**`NodeOutput` is LOSSY vs `NetworkResult` — never treat it as the node's real output.** It is a *render-oriented* projection produced by `convert_result_to_node_output`: there is **no `Geometry2D` variant**, so a `NetworkResult::Geometry2D` (rect/circle/…) collapses to `SurfacePointCloud2D`/`PolyMesh` and its embedded `drawing_plane` is dropped; only a literal `drawing_plane` node yields `NodeOutput::DrawingPlane`. So any API/subsystem that needs a semantic property of the active node's output (its lattice, its construction plane, …) **must derive that onto `NodeSceneData` at `generate_scene` time**, not pattern-match `NodeOutput` after the fact. Precedents on `NodeSceneData`: `unit_cell: Option<UnitCellStruct>` (from `NetworkResult::get_unit_cell`) and `construction_plane: Option<DrawingPlane>` (from `NetworkResult::construction_plane`, which reaches into both `DrawingPlane` and `Geometry2D.drawing_plane`; taken from the interactive/lowest-displayed pin's wire-level result — backs the view-up "from displayed plane" action, issue #349). Adding a field to `NodeSceneData` means updating both its `new()` and the `generate_scene` struct literal — and the direct-construction sites in `tests/structure_designer/multi_output_unit_test.rs`.
