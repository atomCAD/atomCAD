use crate::nodes::edit_atom::commands::add_atom_command::AddAtomCommand;
use crate::nodes::edit_atom::commands::add_bond_command::AddBondCommand;
use crate::nodes::edit_atom::commands::delete_command::DeleteCommand;
use crate::nodes::edit_atom::commands::replace_command::ReplaceCommand;
use crate::nodes::edit_atom::commands::select_command::SelectCommand;
use crate::nodes::edit_atom::commands::transform_command::TransformCommand;
use crate::nodes::edit_atom::edit_atom::DefaultToolState;
use crate::nodes::edit_atom::edit_atom::EditAtomData;
use crate::nodes::edit_atom::edit_atom::EditAtomTool;
...
pub fn edit_atom_data_to_serializable(data: &EditAtomData) -> io::Result<SerializableEditAtomData> {
    ...
    for command in &data.history {
        let (command_type, command_data) = if let Some(select_cmd) =
            command.as_any_ref().downcast_ref::<SelectCommand>()
        {
        ...
        } else if let Some(transform_cmd) = command.as_any_ref().downcast_ref::<TransformCommand>()
        {
            (
                "transform".to_string(),
                serde_json::to_value(transform_cmd)?,
            )
            ...
            return Err(io::Error::new(
                ...
                format!(
                    "Unsupported command type in EditAtomData history: {:?}",
                    command
                ),
            ));
        };
...
pub fn serializable_to_edit_atom_data(
    serializable: &SerializableEditAtomData,
) -> io::Result<EditAtomData> {
    ...
    for cmd in &serializable.history {
        let command: Box<dyn EditAtomCommand> = match cmd.command_type.as_str() {
            ...
            "add_atom" => {
                let add_atom_cmd: AddAtomCommand =
                    serde_json::from_value(cmd.command_data.clone())?;
                Box::new(add_atom_cmd)
            }
            "add_bond" => {
                let add_bond_cmd: AddBondCommand =
                    serde_json::from_value(cmd.command_data.clone())?;
            ...
            "transform" => {
                let transform_cmd: TransformCommand =
                    serde_json::from_value(cmd.command_data.clone())?;
