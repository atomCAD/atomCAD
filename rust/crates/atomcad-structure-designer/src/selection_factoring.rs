...
pub fn create_subnetwork_from_selection(
    ...
    for (i, input) in analysis.external_inputs.iter().enumerate() {
        ...
        let param_node = Node {
            ...
            body_width: crate::node_network::DEFAULT_BODY_WIDTH,
    ...
    for &new_id in id_mapping.values() {
        ...
        for arg in &mut node.arguments {
            let mut new_wires: Vec<crate::node_network::IncomingWire> = Vec::new();
            ...
            for wire in &arg.incoming_wires {
                ...
                if let Some(&mapped_id) = id_mapping.get(&source_id) {
                    ...
                    new_wires.push(crate::node_network::IncomingWire::node_output(
                        mapped_id, pin_idx,
                    ));
                } else {
                    ...
                    {
                        new_wires.push(crate::node_network::IncomingWire::node_output(param_id, 0)); // Parameter output is pin 0
                    }
                }
            }
