...
use crate::node_type::{OutputPinDefinition, Parameter, PinOutputType};
use crate::node_type_registry::{NodeTypeRegistry, allowed_in_zone_body};
...
fn repair_call_sites_for_network(
    ...
    for parent_name in parent_network_names {
        if let Some(parent_network) = node_type_registry.node_networks.get_mut(&parent_name) {
            crate::node_network::walk_all_nodes_mut(parent_network, &mut |node| {
    ...
    for (arg_index, argument) in dest_node.arguments.iter().enumerate() {
        ...
        for incoming in &argument.incoming_wires {
            ...
            let output_pin_index = match incoming.source_pin {
                crate::node_network::SourcePin::NodeOutput { pin_index } => pin_index,
                // Zone-input sources (later phases) aren't validated here.
                crate::node_network::SourcePin::ZoneInput { .. } => {
                    continue;
                }
            };
...
pub fn validate_network(
    ...
    if !validate_parameters(network) {
        network.valid = !crate::node_network::has_interface_residue(&network.validation_errors);
    ...
    crate::invariants::debug_assert_network_invariants(network, node_type_registry);
...
fn validate_zones_recursive(
    ...
    for hof_id in hof_ids {
        ...
        {
            let body = Arc::make_mut(&mut body_arc);
            body.valid = !crate::node_network::has_interface_residue(&body.validation_errors);
        }
