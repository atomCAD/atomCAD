...
use crate::evaluator::atom_op::{eval_output_with_diff, map_atomic, snapshot_atoms};
...
impl NodeData for AtomCutData {
    ...
    fn get_text_properties(&self) -> Vec<(String, TextValue)> {
        vec![
            (
                "cut_sdf_value".to_string(),
                TextValue::Float(self.cut_sdf_value),
            ),
            (
                "unit_cell_size".to_string(),
                TextValue::Float(self.unit_cell_size),
            ),
        ]
    }
    ...
    fn set_text_properties(&mut self, props: &HashMap<String, TextValue>) -> Result<(), String> {
        if let Some(v) = props.get("cut_sdf_value") {
            self.cut_sdf_value = v
                .as_float()
                .ok_or_else(|| "cut_sdf_value must be a float".to_string())?;
        }
        if let Some(v) = props.get("unit_cell_size") {
            self.unit_cell_size = v
                .as_float()
                .ok_or_else(|| "unit_cell_size must be a float".to_string())?;
        }
        Ok(())
    }
...
fn cut_atomic_structure(
    atomic_structure: &mut AtomicStructure,
    cutter_geo_tree_root: &GeoNode,
    scaled_cut_sdf_value: f64,
    unit_cell_size: f64,
) {
