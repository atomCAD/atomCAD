...
use crate::serialization::edit_atom_data_serialization::{
...
impl NodeData for EditAtomData {
    ...
    fn clone_box(&self) -> Box<dyn NodeData> {
        ...
        let cloned_history: Vec<Box<dyn EditAtomCommand>> = self
            .history
            .iter()
...
/// - The selected node is not an edit_atom node
/// - The EditAtomData cannot be retrieved or cast
pub fn get_selected_edit_atom_data_mut(
    structure_designer: &mut StructureDesigner,
) -> Option<&mut EditAtomData> {
    let selected_node_id = structure_designer.get_selected_node_id_with_type("edit_atom")?;
