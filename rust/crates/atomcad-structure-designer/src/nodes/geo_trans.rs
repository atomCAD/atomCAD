...
use crate::evaluator::network_result::{BlueprintData, NetworkResult, runtime_type_error_in_input};
...
impl NodeData for GeoTransData {
    ...
    fn set_text_properties(&mut self, props: &HashMap<String, TextValue>) -> Result<(), String> {
        ...
        if let Some(v) = props.get("rotation") {
            self.rotation = v
                .as_ivec3()
                .ok_or_else(|| "rotation must be an IVec3".to_string())?;
        }
...
impl NodeNetworkGadget for GeoTransGadget {
    fn sync_data(&self, data: &mut dyn NodeData) {
        if let Some(geo_trans_data) = data.as_any_mut().downcast_mut::<GeoTransData>() {
            ...
            geo_trans_data.translation = self
                .unit_cell
