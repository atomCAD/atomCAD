...
- **Math/Programming:** `expr`, `value`, `parameter`, `sequence`, `if` (see below), `array_at`, `array_len`, `array_concat`, `array_append`, `sample_field` (evaluates a `ScalarField` at a `Vec3`; out-of-bounds returns `0.0`, never an error — see `doc/design_scalar_fields.md`), `imat3_rows`, `imat3_cols`, `imat3_diag`, `mat3_rows`, `mat3_cols`, `mat3_diag` (3x3 matrix constructors; `_rows`/`_cols` take three vectors, `_diag` takes one). Stored matrix defaults to identity. Wired input pin overrides the corresponding row/column/diagonal at eval. See `doc/design_matrix_types.md`.
...
- **Geometry 3D (Blueprint outputs):** `cuboid`, `sphere`, `free_sphere`, `extrude`, `half_space`, `drawing_plane`, `facet_shell`, `union`, `intersect`, `diff`, `geo_trans`, `empty`. Primitives take an optional `Structure` input (defaulting to diamond) instead of the old `LatticeVecs`/unit-cell input. `drawing_plane`'s three orientation inputs (`m_index` pin 1, plus `u` pin 5 / `v` pin 6 — in-plane lattice *directions* `[u v w]`) are all **optional** and resolved wired-pin > stored-field > unset, then dispatched through `DrawingPlane::from_spec` (cases A–D: auto axes / pin `u` / pin `u`+`v` / derive Miller from `u × v`). Bad combinations surface as a localized `NetworkResult::Error` — **no validator rule** (the `eval` error is the surfacing). `DrawingPlaneEvalCache` carries the *resolved* orientation so the gadget/editor reflect the effective plane (derived `m`, auto-picked second axis). Design doc: `doc/design_drawing_plane_explicit_axes.md`.
  ...
  `empty` / `empty_2d` are the **empty set** as a first-class primitive
  (`GeoNode::empty_3d` / `empty_2d`), replacing the old "intersect two opposing
  `half_space`s and hope they cancel" hack. Data-free nodes over an SDF that is
  `f64::MAX` everywhere, so they are the `union` identity, the `intersect`
  annihilator, and a `diff` no-op with no special-casing anywhere downstream.
  Why emptiness has to live in the tree rather than be discovered numerically is
  in `atomcad-geo-tree`'s `src/AGENTS.md` ("csgrs has no empty set").
...
- **Run-on-demand search:** `chemisorb` (a thin adapter over
  ...
  **never runs in `eval`**: a search relaxes hundreds of structures, and the
  ...
  despite subnetwork sharing: a foreign or outdated result can only miss. It
  ...
  shape for the next expensive node rather than caching in `eval`.
...
- **Mechanosynthesis build scripts:** `ops_library` (JSON → `OpLibrary`, the one
  producer of that opaque type), `build_script` (JSON → `[BuildStep]`),
  `export_build_script` (an effect node, `[BuildStep]` → a JSON file), and
  `mechanosynth`, which replays the two against a workpiece. `build_step.rs` is
  **not a node** — it is the single place the `BuildStep` record and the engine's
  `Step` convert into each other, and it sits beside `mechanosynth.rs`'s
  `step_record` (which builds the *replayer's* `MechanosynthStep` output) so the
  two records cannot drift. The steps are a plain array of a plain record on
  purpose: `array_concat`, `map`, `filter` and `switch` then work on a build
  script unchanged. `mechanosynth`'s `ops_file` / `build_file` **properties are
...
`default_displayed_output_pins() -> Option<HashSet<i32>>` is the **general** form beside it: an explicit set rather than "all". Both `mechanosynth` nodes answer `Some({2})` — `result` and the base part of `scene` are the same atoms, so showing both draws the workpiece twice, and the scene is what a user scrubbing a build with tools wants to look at (`doc/design_mechanosynth_tools.md`). A node states one hook or the other, never both; the explicit set wins in `apply_default_all_pin_display`, which adds the wanted pins **before** removing the unwanted ones (`set_pin_displayed` drops a node from `displayed_nodes` the moment its pin set empties, which would lose a `Ghost` display type on the way past).

Two things follow the hook and are easy to miss when adding a node that overrides it. The `.cnnd` serializer omits a pin set that equals *the node's own* default rather than a hard-coded `{0}`, and the loader applies that default to every displayed node the file wrote no entry for — which is why the pass runs **after** the nodes are inserted (the default is the node data's to answer). Together they mean a saved default round-trips, a file that chose `{0}` explicitly keeps it, and an old file in which the node sat at the then-default `{0}` loads showing the node's new default.
