...
use crate::evaluator::network_evaluator::{NetworkEvaluationContext, NetworkEvaluator};
use crate::evaluator::network_result::{
...
use atomcad_crystolecule::unit_cell_symmetries::analyze_unit_cell_symmetries;
...
impl NodeData for LatticeSymopData {
    ...
    fn eval<'a>(
        ...
        if let NetworkResult::Error(_) = shape_val {
            ...
            let real_rotation_quat = if rotation_axis.is_none()
                || rotation_angle_degrees.abs() < 1e-6
            {
                // No rotation case
                DQuat::IDENTITY
            } else {
    ...
    fn get_text_properties(&self) -> Vec<(String, TextValue)> {
        let mut props = vec![
            ...
            (
                "rotation_angle_degrees".to_string(),
                TextValue::Float(self.rotation_angle_degrees),
            ),
            (
                "transform_only_frame".to_string(),
                TextValue::Bool(self.transform_only_frame),
            ),
        ];
    ...
    fn set_text_properties(&mut self, props: &HashMap<String, TextValue>) -> Result<(), String> {
        ...
        if let Some(v) = props.get("rotation_axis") {
            self.rotation_axis = Some(
                v.as_vec3()
                    .ok_or_else(|| "rotation_axis must be a Vec3".to_string())?,
            );
        }
        if let Some(v) = props.get("rotation_angle_degrees") {
            self.rotation_angle_degrees = v
                .as_float()
                .ok_or_else(|| "rotation_angle_degrees must be a float".to_string())?;
        }
        if let Some(v) = props.get("transform_only_frame") {
            self.transform_only_frame = v
                .as_bool()
                .ok_or_else(|| "transform_only_frame must be a boolean".to_string())?;
        }
        Ok(())
    }
...
impl NodeNetworkGadget for LatticeSymopGadget {
    fn sync_data(&self, data: &mut dyn NodeData) {
        if let Some(lattice_symop_data) = data.as_any_mut().downcast_mut::<LatticeSymopData>() {
            let real_delta_translation =
                self.frame_transform.translation - self.input_frame_transform.translation;
            lattice_symop_data.translation = self
                .unit_cell
                .real_to_ivec3_lattice(&real_delta_translation);
        }
    }
...
impl LatticeSymopGadget {
    pub fn new(
        ...
        input_frame_transform: Transform,
        unit_cell: &UnitCellStruct,
    ) -> Self {
        let mut ret = Self {
            translation,
            rotation_axis,
            input_frame_transform: Transform::new(
                input_frame_transform.translation,
                input_frame_transform.rotation,
            ),
    ...
    fn apply_drag_offset(&mut self, axis_index: i32, offset_delta: f64) -> bool {
        ...
        let local_axis_dir = match xyz_gadget_utils::get_local_axis_direction(
            &self.unit_cell,
            DQuat::IDENTITY,
            axis_index,
        ) {
