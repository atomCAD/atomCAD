...
// Parameter element constants (`PARAM_ELEMENT_BASE`, `MAX_PARAM_ELEMENTS`,
// `param_atomic_number_to_index` and friends) moved to
// `atomcad_crystolecule::atomic_constants` in Phase 5 of
// `doc/design_rust_crate_split.md` (D7) — `display` reads them per atom and
// must not depend on this crate. Import them from there.
