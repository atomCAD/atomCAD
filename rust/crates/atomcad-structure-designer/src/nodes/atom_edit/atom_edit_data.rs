...
use crate::evaluator::network_result::{Alignment, CrystalData, MoleculeData, NetworkResult};
...
use crate::serialization::atom_edit_data_serialization::{
...
// The `#[flutter_rust_bridge::frb(ignore)]` this type used to carry is gone:
// this crate is not scanned by codegen at all (D11 confines FRB to the root
// crate's `api/`), so there is nothing left to opt out of.
#[derive(Debug)]
pub struct AtomEditData {
    ...
    /// Cross-cell bond metadata: maps a bond to its cell offset and bond order.
    /// Cross-cell bonds are NOT stored as bonds in the diff AtomicStructure;
    /// they exist only in this map and are rendered via ghost bond segments.
    /// Only used when is_motif_mode = true.
    pub cross_cell_bonds: HashMap<BondReference, CrossCellBondInfo>,
...
impl AtomEditData {
    ...
    fn eval_motif_mode<'a>(
        ...
        node_id: u64,
        registry: &crate::node_type_registry::NodeTypeRegistry,
        decorate: bool,
    ...
    fn apply_guideline_decoration(&self, output: &mut AtomicStructure) {
        ...
        if let Some(g) = self.guideline_active() {
    ...
    // =========================================================================
    // Guideline TOOL state machine (issue #368)
    //
    ...
    // the shared `AtomEditSelection`. The guideline value is transient: not
    // serialized, not part of undo/redo, and dropped when the tool variant is
    // replaced (tool switch / node deselect). See
...
impl NodeData for AtomEditData {
    fn provide_gadget(
        ...
        if !self.selection.selected_base_atoms.is_empty()
            ...
            && let Some(result) = structure_designer.get_atomic_structure_from_selected_node()
        {
            for &base_id in &self.selection.selected_base_atoms {
...
pub fn atomic_structure_to_motif(
    ...
    // Convert same-cell bonds from the structure. Cross-cell bonds are NOT
    // stored in the structure; they live only in cross_cell_bonds and are
    ...
    // Add cross-cell bonds from the cross_cell_bonds map.
    // These bonds are not in the structure; their offset and bond order
    // are stored in CrossCellBondInfo.
    for (bond_ref, info) in cross_cell_bonds {
        let atom_a = bond_ref.atom_id1.min(bond_ref.atom_id2);
        let atom_b = bond_ref.atom_id1.max(bond_ref.atom_id2);
        if let (Some(&idx1), Some(&idx2)) = (
            atom_id_to_site_index.get(&atom_a),
            atom_id_to_site_index.get(&atom_b),
        ) {
            // info.offset is offset of max(id1,id2)=atom_b relative to min=atom_a.
            // site_1 (lower id) always gets ZERO; site_2 (higher id) gets the offset.
            bonds.push(MotifBond {
                site_1: SiteSpecifier {
                    site_index: idx1,
                    relative_cell: glam::IVec3::ZERO,
                ...
                site_2: SiteSpecifier {
                    site_index: idx2,
                    relative_cell: info.offset,
                },
                multiplicity: info.bond_order as i32,
            });
        }
    }
...
pub fn generate_ghost_atoms(
    ...
    if !cross_cell_bonds.is_empty() {
        ...
        for (bond_ref, info) in cross_cell_bonds {
            let atom_a = bond_ref.atom_id1.min(bond_ref.atom_id2);
            let atom_b = bond_ref.atom_id1.max(bond_ref.atom_id2);
            // info.offset = offset of max(a,b)=atom_b relative to min(a,b)=atom_a
...
pub fn end_atom_edit_drag(structure_designer: &mut StructureDesigner) {
            ...
            || !recorder.cross_cell_bond_deltas.is_empty())
    {
        structure_designer.push_command(
            crate::undo::commands::atom_edit_mutation::AtomEditMutationCommand {
