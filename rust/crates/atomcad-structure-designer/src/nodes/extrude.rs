...
use crate::evaluator::network_result::runtime_type_error_in_input;
...
impl NodeData for ExtrudeData {
    ...
    fn set_text_properties(&mut self, props: &HashMap<String, TextValue>) -> Result<(), String> {
        if let Some(v) = props.get("height") {
            self.height = v
                .as_int()
                .ok_or_else(|| "height must be an integer".to_string())?;
        }
        ...
        if let Some(v) = props.get("inf").or_else(|| props.get("infinite")) {
            self.infinite = v
                .as_bool()
                .ok_or_else(|| "inf must be a boolean".to_string())?;
        }
