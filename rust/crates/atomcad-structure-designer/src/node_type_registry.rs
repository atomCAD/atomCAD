...
use super::nodes::tag::{tag_get_node_type, untag_get_node_type};
...
use crate::node_network::Argument;
use crate::node_network::IncomingWire;
use crate::node_network::Node;
use crate::node_network::NodeNetwork;
use crate::node_network::SourcePin;
use crate::node_network::{FunctionPinDisposition, function_pin_dispositions};
...
impl NodeTypeRegistry {
    pub fn new() -> Self {
        ...
        // `label`/`fade_depth` are the visual properties written onto matched
        // atoms. Every
        ...
        // label. `fade_depth` (Å) turns the rule's alpha write into a depth
        // ramp — surface atoms get `alpha`, atoms `fade_depth` below the
        // crystal surface are fully transparent (`xray::depth_faded_alpha`,
        // issue #413). See `doc/design_style_rules.md` §"The StyleRule built-in
        // record type def".
        //
        // Four of the seven fields carry editor hints
        ...
        ret.built_in_record_type_defs.insert(
            ...
            RecordTypeDef::from_hinted_fields(
                ...
                vec![
                    ...
                    // Appended last (issue #413) so existing `record_construct`
                    // StyleRule nodes keep their positional wires.
                    (
        ...
        ret.add_node_type(zip_with_get_node_type());
    ...
    pub fn resolve_drag_candidate_type(
        &self,
        node_type: &NodeType,
        data: &dyn crate::node_data::NodeData,
    ) -> NodeType {
        use crate::nodes::{product, record_construct, record_destructure};
    ...
    pub fn populate_custom_node_type_cache_with_types(
        ...
        if node.node_type_name == "record_construct" {
            if let Some(data) =
                node.data
                    .as_any_ref()
                    .downcast_ref::<crate::nodes::record_construct::RecordConstructData>()
            {
                let schema = data.schema.clone();
                let custom = crate::nodes::record_construct::build_node_type_for_schema_with_defs(
                    base_node_type,
                    &schema,
                    record_type_defs,
                    built_in_record_type_defs,
            ...
            if let Some(data) =
                node.data
                    .as_any_ref()
                    .downcast_ref::<crate::nodes::record_destructure::RecordDestructureData>()
            {
                let schema = data.schema.clone();
                let custom = crate::nodes::record_destructure::build_node_type_for_schema_with_defs(
                    base_node_type,
                    &schema,
                    record_type_defs,
                    built_in_record_type_defs,
            ...
            && let Some(data) = node
                .data
                .as_any_ref()
                .downcast_ref::<crate::nodes::product::ProductData>()
        {
            let target = data.target.clone();
            let custom = crate::nodes::product::build_node_type_for_target_with_defs(
                base_node_type,
                &target,
                record_type_defs,
                built_in_record_type_defs,
    ...
    pub fn rekey_record_field_literals(&mut self, def_name: &str, renames: &[(String, String)]) {
        use crate::data_type::RecordType;
    ...
    pub fn find_parent_networks(&self, network_name: &str) -> Vec<String> {
        ...
        for (parent_name, parent_network) in &self.node_networks {
            ...
            let mut found = false;
            crate::node_network::walk_all_nodes(parent_network, &mut |node| {
    ...
    fn compute_apply_custom_type_from_wired_f(
        ...
        use crate::data_type::FunctionType;
        use crate::node_type::OutputPinDefinition;
    ...
    fn resolve_wired_function_signature(
        ...
        ancestors: &[&NodeNetwork],
        ancestor_hof_ids: &[u64],
    ) -> Option<crate::data_type::FunctionType> {
    ...
    fn derive_hof_output_from_signature(
        ...
        src_ft: &crate::data_type::FunctionType,
    ) -> Option<DataType> {
        use crate::data_type::FunctionType;
    ...
    pub fn repair_node_network(&self, network: &mut NodeNetwork) {
        ...
        // Comment anchors (`doc/design_wire_annotations.md` D6): drop every
        // anchor whose target no longer resolves. Runs LAST so it sees the
        // final wire state this function produced — the pin-index remapping
        // above can move an anchored wire's destination slot, and the dangling-
        // wire cleanup can remove the wire outright.
        //
...
fn validate_optionals_in_type(t: &DataType) -> Result<(), String> {
    match t {
        DataType::Optional(inner) => {
            crate::data_type::validate_optional_inner(inner)?;
...
pub fn collect_record_refs_in_node(
    ...
    use crate::nodes::array_at::ArrayAtData;
    use crate::nodes::array_concat::ArrayConcatData;
    use crate::nodes::array_len::ArrayLenData;
    use crate::nodes::closure::ClosureData;
    use crate::nodes::collect::CollectData;
    use crate::nodes::expr::ExprData;
    use crate::nodes::filter::FilterData;
    use crate::nodes::fold::FoldData;
    use crate::nodes::foreach::ForeachData;
    use crate::nodes::map::MapData;
    use crate::nodes::parameter::ParameterData;
    use crate::nodes::product::ProductData;
    use crate::nodes::record_construct::RecordConstructData;
    ...
    let data: &dyn crate::node_data::NodeData = node.data.as_ref();
...
pub fn collect_record_refs_in_network(network: &NodeNetwork, out: &mut HashSet<String>) {
    ...
    for pin in &network.node_type.output_pins {
        if let crate::node_type::PinOutputType::Fixed(t) = &pin.data_type {
...
fn rewrite_record_name_in_registry(
    ...
    for network in node_networks.values_mut() {
        ...
        for pin in network.node_type.output_pins.iter_mut() {
            if let crate::node_type::PinOutputType::Fixed(t) = &mut pin.data_type {
...
pub fn static_match(
    ...
    direction: crate::node_data::DragDirection,
    registry: &NodeTypeRegistry,
) -> bool {
