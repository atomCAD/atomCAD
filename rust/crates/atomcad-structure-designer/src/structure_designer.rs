...
impl StructureDesigner {
    ...
    pub fn mark_selection_changed(
        ...
        {
            crate::nodes::atom_edit::atom_edit::clear_guideline_tool_on_node_deselect(
                self, prev_id,
            );
        }
    }
    ...
    pub fn undo(&mut self) -> bool {
        ...
        if let Some(refresh_mode) = result {
            ...
            crate::nodes::atom_edit::atom_edit::auto_unpick_active_atom_edit_guideline(self);
            true
        } else {
            false
        }
    ...
    pub fn add_node_with_drag_source(
            ...
            )
        {
            ...
            if crate::node_type_registry::static_match_strict(
                &resolved,
                &drag.source_type,
                drag.direction,
                &self.node_type_registry,
    ...
    pub fn connect_wire_scoped(
        ...
        dest_scope_path: &[u64],
        source_node_id: u64,
        source_pin: crate::node_network::SourcePin,
        ...
        if source_scope_depth == 0
            && let crate::node_network::SourcePin::NodeOutput { pin_index } = source_pin
        {
    ...
    pub fn can_connect_wire_scoped(
        ...
        let source_type = match source_pin {
            crate::node_network::SourcePin::NodeOutput { pin_index } => {
            ...
            crate::node_network::SourcePin::ZoneInput { pin_index } => {
                ...
                // to None and reject.
                match &pin.data_type {
                    crate::node_type::PinOutputType::Fixed(t) => t.clone(),
                    _ => return false,
                }
            }
        };

        crate::data_type::DataType::can_be_converted_to(
            &source_type,
            &dest_param_type,
            &self.node_type_registry,
        )
    ...
    pub fn connect_zone_output_wire(
        ...
        while hof_node.zone_output_arguments.len() <= zone_output_index {
            hof_node
                .zone_output_arguments
                .push(crate::node_network::Argument::new());
        }
    ...
    pub fn set_node_network_data_scoped(
        ...
        let old_data_json = if node_type_name != "edit_atom"
            && !crate::nodes::atom_edit::atom_edit::is_atom_edit_family(&node_type_name)
        {
        ...
        if is_expr_node
            && let Some(expr_data) = data
                .as_any_mut()
                .downcast_mut::<crate::nodes::expr::ExprData>()
        {
    ...
    pub fn new_project_direct_editing(&mut self) {
        ...
        if let Some(data) = self.get_node_network_data_mut(node_id)
            && let Some(atom_edit_data) =
                data.as_any_mut()
                    .downcast_mut::<crate::nodes::atom_edit::atom_edit::AtomEditData>()
        {
            atom_edit_data.active_tool = crate::nodes::atom_edit::atom_edit::AtomEditTool::AddAtom(
                crate::nodes::atom_edit::atom_edit::AddAtomToolState::Idle,
            );
        }
        ...
        let displayed_atom_edit_ids: Vec<u64> = network
            ...
            .filter(|&&id| {
                network.nodes.get(&id).is_some_and(|node| {
                    crate::nodes::atom_edit::atom_edit::is_atom_edit_family(&node.node_type_name)
                })
            })
            .copied()
            .collect();
            ...
            if mode {
                            ...
                            .downcast_mut::<crate::nodes::atom_edit::atom_edit::AtomEditData>()
                {
                    atom_edit_data.active_tool =
                        crate::nodes::atom_edit::atom_edit::AtomEditTool::AddAtom(
                            crate::nodes::atom_edit::atom_edit::AddAtomToolState::Idle,
                        );
                }
            }
    ...
    pub fn import_xyz_into_atom_edit(&mut self, file_path: &str) -> Result<(), String> {
        ...
        let selected_node_id =
            crate::nodes::atom_edit::atom_edit::get_selected_atom_edit_family_node_id(self)
...
impl StructureDesigner {
    ...
    pub fn get_selected_wires(&self) -> Vec<crate::node_network::Wire> {
        let network_name = match &self.active_node_network_name {
            Some(name) => name,
            None => return Vec::new(),
        };
    ...
    pub fn select_wires(&mut self, wires: Vec<crate::node_network::Wire>) {
        let network_name = match &self.active_node_network_name {
            Some(name) => name,
            None => return,
        };
    ...
    pub fn add_wires_to_selection(&mut self, wires: Vec<crate::node_network::Wire>) {
        let network_name = match &self.active_node_network_name {
            Some(name) => name,
            None => return,
        };
    ...
    pub fn toggle_wires_selection(&mut self, wires: Vec<crate::node_network::Wire>) {
        let network_name = match &self.active_node_network_name {
            Some(name) => name,
            None => return,
        };
    ...
    pub fn select_nodes_and_wires(
        &mut self,
        node_ids: Vec<u64>,
        wires: Vec<crate::node_network::Wire>,
    ) {
        let network_name = match &self.active_node_network_name {
            Some(name) => name,
            None => return,
    ...
    pub fn begin_gadget_drag_snapshot(&mut self) {
        ...
        if crate::nodes::atom_edit::atom_edit::is_atom_edit_family(&node_type_name) {
            return;
        }
    ...
    pub fn load_node_networks(
        ...
        let param_id_fixes: Vec<crate::network_validator::ParamIdReassignment> = self
            .node_type_registry
            .node_networks
            .values_mut()
            .flat_map(crate::network_validator::dedupe_param_ids_in_network)
            .collect();
    ...
    fn validate_active_network_with_initial_errors(
        &mut self,
        initial_errors: Option<Vec<crate::node_network::ValidationError>>,
...
fn find_node_at_scope<'a>(
    network: &'a NodeNetwork,
    node_ref: &NodeRef,
) -> Option<&'a crate::node_network::Node> {
    let mut current: &NodeNetwork = network;
    for hof_id in &node_ref.scope_path {
        let hof = current.nodes.get(hof_id)?;
        current = hof.zone.as_deref()?;
