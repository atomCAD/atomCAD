...
use crate::preferences::GeometryVisualizationPreferences;
use atomcad_display::surface_point_cloud::SurfacePoint2D;
...
pub fn generate_2d_point_cloud(
    ...
    process_rect_for_point_cloud(
        geometry,
        &(common_constants::REAL_IMPLICIT_VOLUME_MIN
            .round()
            .as_ivec3()
            .xy()
            * geometry_visualization_preferences.samples_per_unit_cell),
        &((common_constants::REAL_IMPLICIT_VOLUME_MAX
            .round()
            .as_ivec3()
            .xy()
            - common_constants::REAL_IMPLICIT_VOLUME_MIN
...
fn process_rect_for_point_cloud(
    ...
    if !should_subdivide_x && !should_subdivide_y {
        ...
        for x in 0..size.x {
            for y in 0..size.y {
                let cell_pos = IVec2::new(start_pos.x + x, start_pos.y + y);
    ...
    let subdivisions = subdivide_rect(start_pos, size, should_subdivide_x, should_subdivide_y);
...
fn process_2d_cell_for_point_cloud(
    ...
    let values: Vec<f64> = corner_points
        .iter()
        .map(|ip| {
    ...
    if values.iter().any(|&v| v >= 0.0) && values.iter().any(|&v| v < 0.0) {
        ...
        point_cloud.points.push(SurfacePoint2D {
