use atomcad_crystolecule::miller::simplify_miller_index;
...
pub fn get_dragged_shift(
    ...
    let distance_along_normal =
        get_closest_point_on_first_ray(&center_pos, &plane_props.normal, ray_origin, ray_direction);
...
pub fn tessellate_shift_drag_handle(
    ...
    let geometry =
        calculate_half_space_geometry(unit_cell, center, miller_index, dragged_shift, subdivision);
    ...
    let handle_start =
        geometry.handle_position - geometry.plane_normal * (SHIFT_HANDLE_CYLINDER_LENGTH / 2.0);
    let handle_end =
        geometry.handle_position + geometry.plane_normal * (SHIFT_HANDLE_CYLINDER_LENGTH / 2.0);
...
pub fn tessellate_plane_grid(
    ...
    let geometry =
        calculate_half_space_geometry(unit_cell, center, miller_index, shift as f64, subdivision);
...
pub fn tessellate_miller_indices_discs(
    ...
    for miller_index in possible_miller_indices {
        ...
        let direction = unit_cell
            .ivec3_miller_index_to_normal(miller_index)
...
pub fn hit_test_center_sphere(
    ...
    // Test central sphere
    sphere_hit_test(&center_pos, CENTER_SPHERE_RADIUS, ray_origin, ray_direction)
}
...
pub fn hit_test_shift_handle(
    ...
    let geometry =
        calculate_half_space_geometry(unit_cell, center, miller_index, shift, subdivision);
