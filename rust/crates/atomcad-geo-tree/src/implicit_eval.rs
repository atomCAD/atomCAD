...
use crate::implicit_geometry::ImplicitGeometry2D;
use crate::implicit_geometry::{BATCH_SIZE, ImplicitGeometry3D};
...
impl ImplicitGeometry2D for GeoNode {
    ...
    fn implicit_eval_2d(&self, sample_point: &DVec2) -> f64 {
        match &self.kind {
            ...
            GeoNodeKind::Union2D { shapes } => Self::union_2d_implicit_eval(shapes, sample_point),
...
impl ImplicitGeometry3D for GeoNode {
    ...
    fn get_gradient(&self, sample_point: &DVec3) -> (DVec3, f64) {
        let epsilon: f64 = 0.001; // Small value for finite difference approximation

        let value = self.implicit_eval_3d(sample_point);
        let gradient = DVec3::new(
            (self.implicit_eval_3d(&(sample_point + DVec3::new(epsilon, 0.0, 0.0))) - value)
                / epsilon,
            (self.implicit_eval_3d(&(sample_point + DVec3::new(0.0, epsilon, 0.0))) - value)
                / epsilon,
            (self.implicit_eval_3d(&(sample_point + DVec3::new(0.0, 0.0, epsilon))) - value)
                / epsilon,
        );
        (gradient, value)
    }
    ...
    fn implicit_eval_3d_batch(
        ...
        match &self.kind {
            ...
            GeoNodeKind::Extrude {
                ...
                plane_to_world_transform,
                infinite,
            } => Self::extrude_implicit_eval_batch(
                *height,
                *direction,
                shape,
                plane_to_world_transform,
...
impl GeoNode {
    ...
    fn extrude_implicit_eval(
        ...
        let sample_horizontal_displacement =
            DVec2::new(direction.x, direction.y) * local_point.z / direction.z;
        let sample_point_2d =
            DVec2::new(local_point.x, local_point.y) - sample_horizontal_displacement;
    ...
    fn extrude_implicit_eval_batch(
        ...
        let world_to_plane = plane_to_world_transform.inverse();
        let mut local_points = [DVec3::ZERO; BATCH_SIZE];
        for i in 0..BATCH_SIZE {
            local_points[i] = world_to_plane.apply_to_position(&sample_points[i]);
        }
        ...
        for i in 0..BATCH_SIZE {
            ...
            let sample_horizontal_displacement =
                horizontal_direction * local_points[i].z * inv_direction_z;
            sample_points_2d[i] =
                DVec2::new(local_points[i].x, local_points[i].y) - sample_horizontal_displacement;
        }
    ...
    fn union_2d_implicit_eval(shapes: &[GeoNode], sample_point: &DVec2) -> f64 {
        shapes
            .iter()
            .map(|shape| shape.implicit_eval_2d(sample_point))
            .reduce(f64::min)
            .unwrap_or(f64::MAX)
    }
    ...
    fn union_3d_implicit_eval(shapes: &[GeoNode], sample_point: &DVec3) -> f64 {
        shapes
            .iter()
            .map(|shape| shape.implicit_eval_3d(sample_point))
            .reduce(f64::min)
            .unwrap_or(f64::MAX)
    }
