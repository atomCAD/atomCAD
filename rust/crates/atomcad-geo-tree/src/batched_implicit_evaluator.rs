...
impl<'a> BatchedImplicitEvaluator<'a> {
    ...
    pub fn flush(&mut self) -> Vec<f64> {
        ...
        if self.use_multithreading && total_points >= MIN_POINTS_FOR_THREADING && thread_count > 1 {
            println!(
                "Using multi-threaded evaluation with {} threads for {} points",
            ...
            if self.use_multithreading {
                println!(
                    "Falling back to single-threaded evaluation (points: {}, threads: {})",
                    total_points, thread_count
                );
            }
    ...
    pub fn flush_multi_threaded(&mut self) -> Vec<f64> {
        ...
        let chunk_results: Vec<Vec<f64>> = self
            .pending_points
    ...
    fn process_chunk(&self, chunk: &[DVec3]) -> Vec<f64> {
        ...
        for chunk_start in (0..padded_len).step_by(BATCH_SIZE) {
            ...
            batch_points[..BATCH_SIZE]
                .copy_from_slice(&padded_chunk[chunk_start..chunk_start + BATCH_SIZE]);
            ...
            self.geo_tree
                .implicit_eval_3d_batch(&batch_points, batch_results);
        }
