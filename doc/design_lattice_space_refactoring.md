...
|---|---|---|---|
| **`HasAtoms`** | Crystal, Molecule | Blueprint | has materialized atoms |
| **`HasStructure`** | Blueprint, Crystal | Molecule | has a structure |
| **`HasFreeLinOps`** | Blueprint, Molecule | Crystal | atoms are not locked to a structure; free movement is legal |
...
- **`get_structure`** — `HasStructure → Structure`. Extracts the structure from a Blueprint or Crystal.
...
|---|---|---|---|---|
| **`structure_move`**, **`structure_rot`** | `HasStructure` | moves geometry only (repositions the cutter) | moves atoms + geometry together | BLOCKED (type error) |
| **`free_move`**, **`free_rot`** | `HasFreeLinOps` | moves geometry only, **amber warning** (cutter is off-structure) | BLOCKED (type error) | moves everything |
...
`atom_edit`, `apply_diff`, `atom_composediff`, `atom_replace`, `add_hydrogen`, `remove_hydrogen`, `relax`, `infer_bonds`, `passivate` — all accept `HasAtoms` and preserve the concrete input type (Crystal stays Crystal, Molecule stays Molecule).

- **`atom_union`** — `Array[HasAtoms] → HasAtoms`. Merges atoms from multiple inputs; raises an error if nuclei are implausibly close. The output concrete type is the "meet" of the input types: all Crystal (with compatible structures) → Crystal; all Molecule → Molecule; mixed Crystal + Molecule → Molecule. Geometry, if present on inputs, is unioned; if only some inputs carry geometry the result keeps the union of those that do.
...
- **`export_xyz`**, **`export_mol`** — input `HasAtoms` (works on Crystal or Molecule).
...
| Old | New |
|---|---|
| `atom_lmove`, `atom_lrot` | `structure_move`, `structure_rot` on `HasStructure` |
| `atom_move`, `atom_rot` | `free_move`, `free_rot` on `HasFreeLinOps` |
| `atom_cut` | `diff` on `Blueprint` followed by `materialize` |
| `atom_union` | `atom_union` on `HasAtoms` (kept, now polymorphic over Crystal/Molecule) + `union` on `Blueprint` for geometric booleans |
...
An abstract input type alone would lose concrete type information downstream: after `Crystal → atom_edit → ???`, the chain's type would collapse to `HasAtoms`, and a subsequent `structure_move` (which needs Crystal) could no longer connect.
...
```
atom_edit:      HasAtoms       →  SameAsInput("input")
structure_move: HasStructure →  SameAsInput("input")
free_move:      HasFreeLinOps     →  SameAsInput("input")
```
...
- **Plain abstract type with runtime downcast** (`HasAtoms → Crystal` allowed with runtime check) — breaks the "errors caught at wire time" guarantee, defers problems to evaluation.

### Migration Notes

- The existing `DataType::Atomic` variant is renamed to `HasAtoms` and redefined from "the concrete atomic-structure type" to "the abstract supertype of Crystal and Molecule".
- Two new concrete `DataType` variants are introduced — `Crystal` and `Molecule` — plus two new abstract variants `HasStructure` and `HasFreeLinOps`. The existing `Geometry` variant is renamed to `Blueprint`. The existing `UnitCell` variant is renamed to `LatticeVecs` and the `unit_cell` node is renamed to `lattice_vecs`.
- All current atom-operation node definitions need their input/output pin declarations updated to `HasAtoms` + `SameAsInput("input")`.
...
- `structure` is present on the two `HasStructure` types (Blueprint, Crystal), absent on Molecule.
- `atoms` is present on the two `HasAtoms` types (Crystal, Molecule), absent on Blueprint.
...
6. **Split Atomic into Crystal / Molecule + abstract types.** Rename `DataType::Atomic` to `HasAtoms` and redefine it as abstract (supertype of Crystal and Molecule). Introduce `HasStructure` and `HasFreeLinOps` abstract types, `can_be_converted_to` rules, and `PinOutputType::SameAsInput` so that polymorphic nodes preserve the concrete input type at the output. Update ~23 atom-operation nodes to use `HasAtoms` + `SameAsInput`. Introduce `CrystalData` and `MoleculeData` payload structs (see Appendix B); both wrap `AtomicStructure`. `atom_fill` now outputs `Crystal`.
This phase has been refined in a separate document and then implmeented: doc/design_crystal_molecule_split.md

7. **Phase transitions, movement nodes, and Flutter API.** `materialize` / `dematerialize` / `exit_structure` / `enter_structure`, `structure_move` / `structure_rot` / `free_move` / `free_rot`. Movement nodes bake transforms directly into atom positions and geo_tree transforms (no carried `frame_transform`). Most of these are obtained by **repurposing existing nodes** (`lattice_move`+`atom_lmove` → `structure_move`, `atom_move` → `free_move`, `atom_fill` → `materialize`, etc.) rather than fresh creations. Delete `atom_trans`. Update `APIDataTypeBase`, regenerate FRB bindings, update Dart UI so the new node set is fully usable end-to-end. Amber warning for off-structure Blueprint moves is deferred. This phase has been refined in a separate document: `doc/design_phase_transitions_and_movement.md`.

8. **Migration script.** `.cnnd` file converter (rename DataType strings, rename node_type_name strings, restructure `atom_fill` into `structure` source + `materialize`). Independent of the code changes — lets existing projects load on the new codebase.
