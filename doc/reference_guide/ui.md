...
2. The single instance is removed and the subnetwork's nodes are copied in where it stood. Each input pin's incoming wire is reconnected to whatever inside the subnetwork consumed the matching `parameter`, and consumers of the instance's output are reconnected to the subnetwork's return node. Room is made for the (usually larger) inlined content the same way it is made for any node that grows: everything right of the instance's old right edge slides right by the width it gained, keeping its spacing and alignment exactly; anything the taller box now reaches is pushed straight down, by the least that clears it. Nodes the growth does not reach are left alone.
...
| Surface transparency | How see-through surfaces are composited — a **drawing** setting, so unlike the three above it changes no geometry and re-extracts nothing. *Sorted lobes (best)* is the default and the only one you should normally use: it draws the lobes back to front for the current camera, then their far side before their near side, which is what makes two overlapping lobes look right from every angle. *Two-pass (unsorted)* and *Single pass (control)* are comparison settings kept for evaluating the default; both misorder overlapping lobes. Nothing to see on an opaque surface, or on one with a single near-spherical lobe — pick a d orbital, or a p orbital viewed down an axis where the lobes overlap. |
...
| Show tool collision envelopes | Draws a wireframe **cage** of every bound tool's collision envelope on every displayed [`mechanosynth`](nodes/atomic.md#mechanosynth) node: the cone from its apex to the rim where the cylinder begins, and 12 Å of the cylinder beyond it, as sixteen meridians and three rings. The cage's apex sits at the tool-side reaction point of the tool's **nearest visit** — the current step when it is that tool's, else its next `tip` step in the script, else its last — and the cage is posed with the tool, so it descends with the tool and sits on the site at the reaction. What the panel's approach line calls *clear*, *tilted* or *blocked* is this cone against the atoms around the site (see [*Scrubbing inside a step*](nodes/atomic.md#scrubbing-inside-a-step)). The envelope is the library's claim about the instrument rather than the shape of the wired molecule, so it is usually wider than the atoms inside it. Off by default: it is a way of looking at any build rather than a property of one, which is why it is a preference and not a node setting. Toggling it redraws the scene and re-evaluates nothing. |
| Tool envelope color (RGB) | The cage's line colour. Default `255, 160, 0`, an orange that reads against both a passivated surface and the bulk. |
