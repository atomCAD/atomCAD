...
Some node types ship with **built-in record defs** — schemas baked into the application that you don't have to author yourself. Four ship today:
...
- `MechanosynthStep = {index: Int, count: Int, op: String, note: String, method: String, phase: String, layer: Int, site: Int, t: Vec3}` — what [`mechanosynth`](./atomic.md#mechanosynth)'s `step` output pin says about the last build step applied. Unlike the three above it is *produced* by a node rather than consumed by one: destructure it, or read a field with `expr`, to drive a `switch` or a caption from the build script's own metadata. See [*The `step` output pin*](./atomic.md#the-step-output-pin).

Built-in defs share one namespace with user defs and participate in the same bare-identifier lookup (so `ElementMapping`, `Patch` and `MechanosynthStep` work as type expressions in `expr` parameters and in the schema dropdowns), but they cannot be edited or deleted from the User Types panel, and the User Types panel will reject attempts to create, rename, or delete a user def with the same name.
