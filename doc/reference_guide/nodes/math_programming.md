...
- `MechanosynthStep = {index: Int, count: Int, op: String, note: String, method: String, phase: String, layer: Int, site: Int, t: Vec3, r: Mat3}` — what [`mechanosynth`](./atomic.md#mechanosynth)'s `step` output pin says about the last build step applied. `method` is the **operation's** kind — `tip`, `bulk` or `spontaneous` — read from the wired library rather than from the step, which no longer states one. Unlike the three above it is *produced* by a node rather than consumed by one: destructure it, or read a field with `expr`, to drive a `switch` or a caption from the build script's own metadata. `t` and `r` together are the step's placement, so a gadget wired off this pin can be oriented as well as positioned. See [*The `step` output pin*](./atomic.md#the-step-output-pin).
