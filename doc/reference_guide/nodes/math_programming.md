...
Some node types ship with **built-in record defs** — schemas baked into the application that you don't have to author yourself. Five ship today:
...
- `MechanosynthStep = {index: Int, count: Int, op: String, note: String, method: String, phase: String, layer: Int, site: Int, t: Vec3, r: Mat3, tool_type: String, tool_state: String, agent: String, time: Float, tool_r: Mat3, tool_t: Vec3, approach: Float, contact: Float}` — what [`mechanosynth`](./atomic.md#mechanosynth)'s `step` output pin says about the last build step applied. `method` is the **operation's** kind — `tip`, `bulk` or `spontaneous` — read from the wired library rather than from the step, which no longer states one; the last three are the tool model's, appended so existing `record_destructure` wires keep their positions. `tool_type` is a `tip` operation's instrument and `tool_state` the state that instrument is in **after** the step, so a caption can read *habst_tool · spent*; `agent` is a `bulk` operation's species or energy. Each is the empty string where it does not apply, as every other absent field is. Unlike the three above it is *produced* by a node rather than consumed by one: destructure it, or read a field with `expr`, to drive a `switch` or a caption from the build script's own metadata. `t` and `r` together are the step's placement, so a gadget wired off this pin can be oriented as well as positioned. The last five are the **trajectory**'s, appended for the same reason: `time` is where inside the step the outputs were taken, `tool_r` / `tool_t` are the moving tool's frame at that moment, and `approach` / `contact` are how much room the visit had — the sweep's clearance in ångström (negative when the site is blocked) and the closest the tool came to anything on the way, as a ratio of covalent radii. See [*The `step` output pin*](./atomic.md#the-step-output-pin) and [*Scrubbing inside a step*](./atomic.md#scrubbing-inside-a-step).
- `BuildStep = {op: String, t: Vec3, r: Mat3, note: String, phase: String, layer: Int, site: Int}` — **seven fields**; one step of a mechanosynthesis build script as it travels the network: which operation, and the rigid transform placing it (`p_workpiece = r · p_local + t`). [`build_script`](./atomic.md#build_script) loads an array of these, [`mechanosynth`](./atomic.md#mechanosynth) replays one, and [`export_build_script`](./atomic.md#export_build_script) writes one back out. Because it is a plain record, a build script concatenates with `array_concat`, reshapes with `map` and `filter`, and can be written by hand with `record_construct` — which is the whole point of it being a record rather than a wrapper type.

  It is deliberately **distinct from `MechanosynthStep`**: that one is the replayer's readout and carries replay provenance (`index`, `count`) an authored step does not have. Absent fields take the build file's own defaults — an identity `r`, empty strings, `-1` for `layer` and `site`.

Built-in defs share one namespace with user defs and participate in the same bare-identifier lookup (so `ElementMapping`, `Patch`, `MechanosynthStep` and `BuildStep` work as type expressions in `expr` parameters and in the schema dropdowns), but they cannot be edited or deleted from the User Types panel, and the User Types panel will reject attempts to create, rename, or delete a user def with the same name.
