...
- **Orphaned (dropped) ghosts** — patch-ghosts with no real twin. A few are expected at the true reconstruction edges. A *high* count means the collars aren't meeting substrate — usually a **registration mismatch** (`target` doesn't share the patch's lattice and motif registration), or an `origin` offset that slid the reconstruction off the surface.
