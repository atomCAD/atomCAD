...
- **Parametric, composable designs.** atomCAD designs are parametric and composed as visual node networks, enabling non-destructive editing. Custom node types can be created by defining subnetworks. The node network includes functional-programming elements — the higher-order-function nodes (`map`, `filter`, `fold`, `foreach`) each carry an *inline body region* where you author the per-element computation directly inside the node, and a `closure` node turns such a body into a reusable first-class function value that can be passed around and called (via `apply` or an HOF's `f` pin) — and the node language is Turing-complete.
- **Surface reconstructions.** A one-click (100) 2×1 dimer reconstruction is built into the `materialize` node (cubic diamond). For arbitrary reconstructions, the **surface patch** nodes (`patch_build` + `patch_latticefill`) let you author a per-cell rearrangement once from a hand-built example and tile it commensurately across a crystal face, welding it into the bulk automatically.
