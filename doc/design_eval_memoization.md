...
#### The Redundancy tab becomes a *residual* view — corrected after Phase 3

**This subsection originally claimed that `lookups`, and therefore
`redundancy_factor` and `wasted_ns`, survive the memo numerically
unchanged, and proposed relabelling the column `Wasted` → `Saved` to
express the change of tense. The Phase 3 measurement showed that claim is
false, and the relabel with it.** The reasoning below is what replaced it;
the original is preserved in the bullets so the mistake is legible.

The mistake: `lookups` measures demand, but *demand is itself generated by
re-evaluation*. `materialize#8`'s twelve lookups were never twelve distinct
consumers — they were the whole cone above it being re-walked twelve times.
Once the intermediate nodes are memoized their `eval` is skipped, so they
never re-issue their own downstream pulls, and the collapse compounds
top-down. Measured on the same design after Phase 3: `materialize#8`
12 → **1** lookup, `geo.1-precursor_proxy#6` 12 → 1, `atom_edit#113`
11 → 3, `freeze#1` 12 → 2. Lookups survive exactly where there really are
several distinct consumers within one environment, which is the minority
of the original count.

Three consequences:

- **`redundancy_factor` does collapse**, to ~1.0× everywhere. That is not a
  factor defined over the wrong thing — it is the correct reading of a
  different question. With the memo on the tab answers *"is any redundancy
  left?"*, and all-1.0× is the acceptance criterion passing.
- **`wasted_ns` collapses with it**, to ~0. So the `Wasted` → `Saved`
  relabel is **rejected**: it would print `Saved: 0.0 ms` on a pass where
  the memo saved six seconds, which is the opposite of the truth. The
  column keeps the name `Wasted` in both switch positions, and the footnote
  says which of the two readings applies.
- **"What did the memo do?" is answered elsewhere** — by the memo
  statistics block below (hits over requests) and by A/B-ing two rows in
  the history ring. That is the honest place for it: those numbers are
  about the memo, where the Redundancy columns are about the network.

`wasted_ns`'s doc comment must still be corrected — it claimed the value
reaches zero once the memo works, for the wrong reason (it said `lookups`
do not fall; they do). It reaches zero, and the reason is the collapse
above.

**`evaluations` against `distinct_envs` is the actual regression test**,
and the reader should not have to diff two columns to run it. The count of
unflagged rows where `evaluations > distinct_envs` goes in the footnote —
`EvalProfile::unmemoized_offender_count`. With the memo on it reads zero,
and that single number *is* this design's acceptance criterion. With the
memo off it is meaningless (every shared node offends), so the footnote
shows the projected saving there instead.
...
### Phase 4 — Reading the result (D10, presentation only). Done
...
change nobody reviews. That split earned itself: the first profile is what
showed the `Wasted` → `Saved` relabel to be wrong (see the corrected
subsection in D10).

**As built.** `APIRefreshProfile` carries an `APIMemoCounts` per row, so an
A/B pair is two ring rows each with its own numbers and a `·memo off` tag
on the mode cell; the Phases tab gains a `Memo hit/req` column beside
`CSG hit/lookup` and a statistics bar above the ring (peak entries and
bytes against the budget, end size, epoch drops, declined inserts,
estimator time, and an amber warning naming the preference when the LRU
evicted anything). `APIEvalProfile` gains `unmemoized_offenders`, and the
Redundancy footnote switches between "no unexplained repeats" and the
projected saving depending on the switch. **The relabel was dropped** (D10
as corrected). Two stale sentences were fixed in the same change: the
By-node footnote's "equal until the evaluation memo lands", and
`wasted_ns`'s doc comment.

The estimator-time counter D10 marks optional ships as part of the
`MemoCounts` block under the name `insert_ns`, rendered only when it
reaches 1 ms — so it stays invisible until it is worth looking at, which is
the condition D10 attached to keeping it.
...
- The footnote's offender count excludes `iterator`, `cycle`,
  ...
  Covered by the subnetwork fixture, where the instance row genuinely has
  `evaluations > distinct_envs` and must not be counted.
- A history ring holding one memo-off and one memo-on row carries
  distinguishable counters — the flag rides on the row, not on the current
  state of the switch, because by the time you read the ring the switch
  says nothing about the row you are comparing against.
- Predicted (`total_distinct_envs`) against actual peak entries: actual
  never exceeds predicted on a fixture. A free check that the memo and the
  profiler key on the same thing.
- The footnote's two readings come from two different numbers (offender
  count with the memo on, projected saving with it off), asserted on the
  data rather than on the rendered string.

The relabel test is gone with the relabel. The panel's own rendering is
left to the manual walkthrough, per the project's preference for manual
verification of thin editor UI over mandated widget tests.
