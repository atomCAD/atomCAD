...
### Phase 3 — Flutter UI + API ✅ DONE

1. `APIDataTypeBase::Optional` + conversion in both directions (`flutter_rust_bridge_codegen generate`). Represented **like `Iter`**: a first-class base with one child (the inner type), not a `bool` modifier — this matches §7's "one entry in `children`" and reuses the existing structural-type machinery. `api_data_type_to_data_type` rejects the four ill-formed inners via `validate_optional_inner`; `data_type_to_api_data_type` has a dedicated `Optional` branch before the flat match.
2. `DataTypeInput` exposes Optional as a **dropdown base entry** (not a separate checkbox), gated by a new `allowOptional` param so it appears **only in the `SchemaEditor`** field-type picker. The inner type is edited via the same `showTypeEditorDialog` dialog as Iter (a nested `DataTypeInput` labeled "Inner Type"). Nesting guard: the nested editor is passed `optionalInner: true`, which hides `Optional`/`Iter`/`Unit`/`None` from its dropdown so an ill-formed inner cannot be authored. The outer Array checkbox is **hidden** for Optional — `Array[Optional[T]]` is not authorable; wrap the array *inside* the Optional (`Optional[Array[T]]`) instead.
3. `record_construct` literal editor: the existing `LiteralFieldsEditor` already implements the clearable/tri-state affordance (Stored / Placeholder-`(unset)` / Wired). The only backend change needed was peeling `Optional[T] → T` in `get_record_construct_fields` (via `record_field_pin_type()`) so the inner simple type flows through and renders an input. "Unset" = no `literal_values` entry (the editor's Clear button removes it). Default = unset (a fresh node has no entry).
4. `flutter analyze` clean (no new issues). Manual pass left to the user: declare a record def with `Optional[Bool]` / `Optional[Float]` fields, build construct/destructure chains, leave optional construct pins unwired (unset) and confirm the destructured `T` output reads `None` and feeds an ordinary consumer; set a literal and confirm it overrides.
