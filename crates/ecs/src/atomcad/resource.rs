...
pub trait ResourceManager {
    ...
    fn get_resource_mut<R: Resource<Mutability = Mutable>>(&mut self) -> Option<Mut<'_, R>>;
    ...
    /// Gets a mutable reference to the resource of type `R`.
    ///
    /// # Panics
    /// Panics if there is no resource of type `R`.
    ...
    ///
    /// **Note:** If the resource does not exist, the closure will not be called.
    fn edit_resource<R: Resource<Mutability = Mutable>>(
        &mut self,
        f: impl FnOnce(Mut<R>),
    ) -> &mut Self {
    ...
    T: ContainsWorld,
{
    ...
    fn insert_non_send<R: 'static>(&mut self, resource: R) -> Option<R> {
        let world = self.world_mut();
        let obj = world.remove_non_send::<R>();
        world.insert_non_send(resource);
        obj
    }

    fn remove_non_send<R: 'static>(&mut self) -> Option<R> {
        self.world_mut().remove_non_send::<R>()
    }

    fn get_non_send<R: 'static>(&self) -> Option<&R> {
        self.world().get_non_send::<R>()
    }
