...
impl PeriodicTable {
    pub fn new() -> Self {
        ...
        element_reprs[Element::Empty as usize] = ElementProperties {
...
mod tests {
    ...
    fn test_periodic_table_initialization() {
        ...
        // Should have entries for all elements from 0 to 118
        assert_eq!(table.element_reprs.len(), 119);

        // Check if the first element (Hydrogen) has the expected radius
        assert!(
            (table.element_reprs[Element::Hydrogen as usize].radius - 1.10).abs() < f32::EPSILON
        );
    ...
    /// Test that chemical symbols correctly convert to elements
    #[test]
    fn test_element_from_symbol() {
        // Test single-letter symbols
        assert_eq!(Element::from_symbol("H"), Some(Element::Hydrogen));
        assert_eq!(Element::from_symbol("C"), Some(Element::Carbon));
        assert_eq!(Element::from_symbol("N"), Some(Element::Nitrogen));
        assert_eq!(Element::from_symbol("O"), Some(Element::Oxygen));
        assert_eq!(Element::from_symbol("F"), Some(Element::Fluorine));
        assert_eq!(Element::from_symbol("I"), Some(Element::Iodine));
        assert_eq!(Element::from_symbol("W"), Some(Element::Tungsten));
        assert_eq!(Element::from_symbol("U"), Some(Element::Uranium));

        // Test two-letter symbols
        assert_eq!(Element::from_symbol("He"), Some(Element::Helium));
        assert_eq!(Element::from_symbol("Li"), Some(Element::Lithium));
        assert_eq!(Element::from_symbol("Na"), Some(Element::Sodium));
        assert_eq!(Element::from_symbol("Fe"), Some(Element::Iron));
        assert_eq!(Element::from_symbol("Au"), Some(Element::Gold));
        ...
        // Test round-trip with from_atomic_number
        assert_eq!(Element::from_symbol("Ag"), Element::from_atomic_number(47));
    }
