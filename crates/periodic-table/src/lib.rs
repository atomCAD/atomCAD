...
mod tests {
    ...
    /// Test ElementRepr memory layout
    #[test]
    fn test_element_repr_memory_layout() {
        // Verify the size of ElementRepr is what we expect
        assert_eq!(std::mem::size_of::<ElementRepr>(), 16);

        // Verify the alignment
        assert_eq!(std::mem::align_of::<ElementRepr>(), 4);
    }
}
