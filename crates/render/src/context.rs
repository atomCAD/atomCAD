...
use crate::error::{RenderContextError, SurfaceAcquireError};
...
impl RenderContext {
                ...
                wgpu::util::new_instance_with_webgpu_detection(wgpu::InstanceDescriptor {
                    ...
                    backend_options: wgpu::BackendOptions {
                        ...
                        gl: wgpu::GlBackendOptions {
                            ...
                            // Let wgpu decide whether the driver's debug functions can be
                            // trusted; some (e.g. Mali) crash when they are used.
                        ...
                        dx12: wgpu::Dx12BackendOptions {
                            ...
                            latency_waitable_object:
                                wgpu_types::Dx12UseFrameLatencyWaitableObject::from_env()
                                    .unwrap_or_default(),
                            // We do not use passthrough shaders, so there is no shader model to
                            // force, and we ship no Agility SDK of our own.  The latter can still
                            // be pointed at one via the environment, if desired.
                            force_shader_model: wgpu::ForceShaderModelToken::default(),
                            agility_sdk: wgpu_types::Dx12AgilitySDK::from_env(),
                        },
                    ...
                    // The GL backend needs the platform display connection to be able to present,
                    // most notably under Wayland.  The window we are about to create the surface
                    // from is the very same one, as required.
    ...
    /// Reapplies the current configuration to the surface.
    ///
    /// Needed when the surface reports its configuration as outdated, even though nothing about
    /// the requested configuration has changed from our point of view.
    ...
    pub fn frame_encoder(
        &mut self,
        label: Option<&str>,
    ) -> Result<&mut FrameEncoder, SurfaceAcquireError> {
        // There can only be one frame encoder per frame.
        if self.current_frame.is_none() {
            // Get the next texture in the swapchain.  A suboptimal texture no longer matches the
            // properties of the surface, but is still perfectly usable, so we draw to it anyway
            // rather than dropping the frame; the next resize will reconfigure the surface.
            let buffer = match self.surface.get_current_texture() {
                wgpu::CurrentSurfaceTexture::Success(buffer)
                | wgpu::CurrentSurfaceTexture::Suboptimal(buffer) => buffer,
                wgpu::CurrentSurfaceTexture::Timeout => return Err(SurfaceAcquireError::Timeout),
                wgpu::CurrentSurfaceTexture::Occluded => return Err(SurfaceAcquireError::Occluded),
