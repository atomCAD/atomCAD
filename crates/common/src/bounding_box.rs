...
impl BoundingBox {
    ...
    /// Computes the 1D intersection times for a directed line segment.
    /// Imagine a point moving the number line, starting at `origin` (when
    /// t=0) and moving at some `speed`.  If the point ever crosses the value
    /// `min` or `max`, then this function will return Some((t_min, t_max)),
    /// such that `origin + speed * t_min = min` adn `origin + speed * t_max =
    /// max`. These time values may be negative if needed to satisfy those
    /// relations.
    ///
    /// If `speed` is zero, but the `origin` is between `min` and `max`, then
    /// this function returns `Some((f32::NEG_INFINITY, f32::INFINITY))`.
    /// This signifies that, although intersection will not happen, the point
    /// *is* between `min` and `max` for all times.
    ///
    /// If `speed` is zero and `origin` is outside of `[min, max]`, then this
    /// function returns `None` to indicate that intersection is impossible
    /// and the point will never be in the provided range.
    fn intersection_times(origin: f32, speed: f32, min: f32, max: f32) -> Option<(f32, f32)> {
        ...
        // along that axis, the ray runs parallel to the slab. We represent
        // this as "intersecting" the slab from negative infinity to positive
        // infinity.
        else if origin >= min && origin <= max {
            Some((f32::NEG_INFINITY, f32::INFINITY))
        }
        // If the direction component is zero and the origin is outside the
        // bounding slab along that axis, the ray will never intersect the
        // slab.
        else {
            None
        }
    ...
    // Given a raycast defined by target_point = origin + velocity * t,
    // returns the values of t where the ray enters and exits the box (in that
    // order).  If the ray fails to hit this box, returns None. Note that the
    // time values may be negative, but tmin will always be <= tmax.
