...
static PROCESS_NAME: LazyLock<RwLock<OsString>> =
    LazyLock::new(|| RwLock::new(crate::platform_impl::get_process_name()));
...
pub fn set_process_name(name: &OsStr) {
    crate::platform_impl::set_process_name(name);
    *PROCESS_NAME.write().expect("Poisoned RwLock") = name.to_owned();
}
