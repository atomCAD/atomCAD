...
use render::{RenderContext, RenderContextError, SurfaceAcquireError};
    ...
    fn window_event(&mut self, _event_loop: &ActiveEventLoop, event: &WindowEvent) {
        match event {
            WindowEvent::RedrawRequested => match self.render() {
                Ok(_) => {}
                // The window isn't visible, or the swapchain is busy.  Skip the frame; we will be
                // told to redraw again once the window becomes visible (see below).  Notably,
                // macOS can report a newly created window as occluded before it is shown.
                Err(RenderContextError::SurfaceError(
                    error @ (SurfaceAcquireError::Occluded | SurfaceAcquireError::Timeout),
                )) => {
                    log::debug!("Skipping frame: {error}");
                }
                // The surface changed underneath us, so reconfigure it and try again.
                Err(RenderContextError::SurfaceError(SurfaceAcquireError::Outdated)) => {
                    if let Some(rc) = self.render_context.as_mut() {
                        rc.reconfigure_surface();
            ...
            WindowEvent::Occluded(false) => {
                // Draw any frame that was skipped while the window was hidden.
                if let Some(rc) = self.render_context.as_ref() {
                    rc.window().request_redraw();
                }
            }
