...
impl<'app> Application<'app> {
    fn new(app: &'app mut App, startup_action: StartupAction) -> Self {
        ...
        app.add_systems(First, move |mut ev_app_exit: MessageWriter<AppExit>| {
...
impl ApplicationHandler for Application<'_> {
    ...
    fn window_event(
        ...
        if let Some(splash_screen) = &mut self.splash_screen
            && splash_screen.get_window_id() == Some(window_id)
        {
